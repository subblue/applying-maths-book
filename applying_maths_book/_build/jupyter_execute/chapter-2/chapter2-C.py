...
# 
# Although we can form the complex conjugate and calculate $F^*F=f^2(1 +e^{-i\pi(h+k)})(1 +e^{+i\pi(h+k)})$ in this case it is not necessary because $h, k, l$ are integers and we know that $e^{n i\pi}=e^{-n i\pi}=(-1)^2$. Thus when $h+k$ is an even number $F=2f$ and when it is not $F=0$. The value of $l$ has no effect thus refections from indices, for example, $131, 132, 133$ etc. all have the same intensity, and similarly $122,123,124$ etc. are all missing with zero structure factor.
# 
