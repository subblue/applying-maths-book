...
# 
# $$\displaystyle g(k)=\frac{1}{\sqrt{2\pi}}\int_0^\infty \frac{\exp(-t/\tau)}{\tau} \exp(-ikt)dt$$
# 
...
# 
# The absolute value of a complex number $z$, is $\sqrt{z^*z}$ and its square is clearly $z^*z$, where $z^*$ is the conjugate of the complex number formed by replacing $i$ by $-i$. Doing the same with $g(k)$ gives 
# 
# $$\displaystyle |g(k)|^2= \frac{1}{2\pi (k^2\tau^2+1)}$$
# 
...
# 
# $$\displaystyle \begin{align} \frac{i}{\sqrt{2\pi}}\frac{1-e^{i(\omega_0 - \omega)t_0}}{\omega_0 - \omega}& = \frac{1}{\sqrt{2\pi}(\omega_0 - \omega)}\left (i-ie^{i(\omega_0 - \omega)t_0}  \right )\\& = \frac{1}{\sqrt{2\pi}(\omega_0 - \omega)}\left (i-i\cos((\omega_0 - \omega)t_0)+\sin((\omega_0 - \omega)t_0)  \right ) \end{align}$$
# 
# The square of the absolute value is obtained by multiplying the complex conjugate $g^*$ with $g$:
# 
# $$\displaystyle  \begin{align}|g(\omega)|^2 = g(\omega)^*g(\omega)&=\left[\frac{-i}{\sqrt{2\pi}}\frac{1-e^{-i(\omega_0 - \omega)t_0}}{\omega_0 - \omega} \right]\left[\frac{i}{\sqrt{2\pi}}\frac{1-e^{i(\omega_0 - \omega)t_0}}{\omega_0 - \omega}\right]\\&=\frac{1-\cos((\omega_0-\omega)t_0)}{\pi (\omega_0-\omega)^2}\end{align}$$
# 
...
# 
# which can be evaluated by hand or using SymPy. By hand this is made easier by converting the sin to exponential first. 
...
# 
# $$\displaystyle \chi= \frac{1}{\sqrt{2\pi}}\sum_{n=-\infty}^{\infty}e^{ikna}$$
# 
# then take pairs of functions with index values $\pm n$, starting at $\pm 1$, and then add the $n$ = 0 term and evaluate the result using the relationship $\displaystyle 2 \cos(nx) = e^{inx} + e^{−inx}$. This gives
# 
# $$\displaystyle \chi(k)= \frac{1}{\sqrt{2\pi}}[1+2\sum_{n=1}^\infty \cos(nka)]$$
# 
