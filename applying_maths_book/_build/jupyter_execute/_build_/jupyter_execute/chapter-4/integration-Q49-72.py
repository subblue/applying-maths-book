...
# 
# where $y'$ is the derivative. Comment on the surface area with respect to the volume. Does this make sense ?
# 
...
# (a) Evaluate 
# 
# $$\displaystyle F=\frac{d}{dp}\int_a^b e^{-px}dx$$
# 
# by integrating first then differentiating the result, then doing the calculation in reverse order.
# 
# (b) Do the same with 
# 
# $$\displaystyle F =\frac{d}{dp}\ln\left( \int_a^b e^{-px}dx \right)$$
# 
# and show that this is the average $\langle x \rangle$ of the exponential distribution $e^{-px}$.
# 
...
# 
# where $T$ is the temperature in K, and $k_B$ Boltzmann's constant = $1.38 \cdot 10^{-23} \,\mathrm{J \,K^{-1}}$ and $E \ge 0$.
# 
...
# 
# (b) Calculate also the dispersion 
# 
# $$\displaystyle \sigma_E^2 = \langle E^2\rangle  -\langle E\rangle^2$$
# 
...
# 
# $$\displaystyle \frac{d\ln(k(T)}{d{1/T}}=-\frac{E_a}{k_B}$$
# 
# Show that this is $\displaystyle \frac{dk(T)}{d{T}}=\frac{E_ak(T)}{k_BT^2}$. 
# 
# (c) Collision theory defines the rate constant as 
# 
# $$\displaystyle k(T)=2\sqrt{\frac{2}{\pi \mu}}\left( \frac{1}{k_BT} \right)^{3/2}\int_0^\infty E\,\sigma(E)e^{-E/k_BT}dE$$
# 
...
# 
# (e) Calculate the mean momentum $\langle p \rangle$ and $\langle p^2 \rangle$ and confirm the uncertainty principle as
# 
# $$\displaystyle \sqrt{\langle p^2 \rangle}\sqrt{\langle x^2 \rangle} \ge \hbar/2$$
# 
...
# 
# where $\beta = 1/k_BT$ and $k_B$ is the Boltzmann constant and $T$ the temperature. This equation is, of course, not such an accurate model for every molecule but if the vibrational energies are small compared to $k_BT$ then it is a good approximation.
# 
...
# 
# where $N_n$ is the normalization and $L$ the length of the box and $n$ the quantum number which has values $n = 1, \,2, \,3, \cdots$. The wavefunction is defined from $0 lt x \lt L$ and is zero outside these limits.
# 
...
# 
# where $m$ is the mass of the particle in this case an electron.
# 
...
# 
# $$\displaystyle F_{n,m}=\bigg| \int_{-\infty}^\infty \psi_{a,n}^*(x-x_a)\psi_{b,m}(x-x_b)dx\bigg|^2 \equiv \bigg| \langle n|m \rangle \bigg |^2 $$
# 
...
# 
# $$\displaystyle \psi_n(x-x_o)=\sqrt{\frac{1}{2^2n!}  }\left( \frac{\alpha}{\pi} \right)^{1/4}H_n\left( (x-x_0)\sqrt{\alpha} \right)e^{-\alpha(x-x_0)^2/2} \tag{44}$$
# 
...
# 
# where $X = \alpha(x_n - x_m)^2$ is the dimensionless displacement between the two electronic states. What are the units of $\alpha$ if $X$ is dimensionless?
# 
...
# 
# $$\displaystyle I(t)=I_0\exp\left(-k_t t-\frac{c}{c_0}\sqrt{\pi k_f t}\right)$$
# 
# where $c$ is the concentration of acceptor molecules, and $c_0 = 3\pi N/4R_0^3$ is a constant. $R_0$ is the characteristic energy transfer distance between donor and acceptor which is a measure of the overlap of the donor's emission with the acceptor's absorption spectrum. Experimental measurements confirm this theoretical expression.
# 
...
# 
# The volume element for integrating over all coordinates $r,\, \theta, \, \phi$ is $r^2\sin(\theta)drd\theta d\phi$. However, as there are no terms in products $r\theta, \,\theta\phi$, or $r\phi$, such as $\sin(r\theta)$, this allows the integral to be separated into three parts;
# 
# $$\displaystyle \qquad \qquad 
# \begin{align}N=&\int_0^\pi\int_0^{2\pi}\int_0^\infty \psi^*\psi r^2\sin(\theta)dr d\theta d\phi\\=& \int_0^\pi \sin(\theta)d\theta\,\int_0^{2\pi} d\phi\,\int_0^\infty r^2\psi(r)^*\psi(r)dr\end{align} \qquad \qquad \qquad \qquad  \text{(46)}$$
# 
...
# 
# $$\displaystyle W(E_\theta)dE_\theta =e^{-E_\theta /k_BT}dE_\theta $$
# 
...
# 
# If there are $N$ dipoles, then the fraction that lie in the angular range $\theta \to \theta + d\theta$ is just $N/4\pi$ times this quantity because the total surface area of the unit sphere is $4\pi$, therefore the fraction at angle $\theta$ is 
# 
# $$\displaystyle P(\theta)d\theta = (N/2) \sin(\theta)W(E_\theta)d\theta$$
# 
# The calculation is shown in the left-hand part of Fig. 20.
# 
...
# 
# where $I$ is the moment of inertia. The probability distribution describing the chance of a molecule having energy $E$, is obtained from the Boltzmann equation 
# 
# $$\displaystyle P(E_J)dE_J = g_J e^{-E_J/k_BT}dE_J$$
# 
# where T is the temperature (K), $k_B$ Boltzmann's constant ($1.38 \cdot 10^{-23}\,\mathrm{ JK^{-1}}$), and $g_J$ is the degeneracy of level $J$; in this case this is $2J + 1$.
# 
