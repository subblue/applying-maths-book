...
# 
# <img src='differen-fig61.png' alt='Drawing' style='width:300px;'/>
# 
...
# 
# ### Q87 answer
# The function to minimize is $f(x, y) =\sqrt{ x^2 + y^2}$ and the constraint is $g(x, y) = y -\sqrt{ (4 - x)^3/x}$. Taking the derivatives and putting values into eqn. (37) gives $\displaystyle -\frac{x}{f}+\frac{\lambda}{2y}\left( -\frac{3(4-x)^2}{x}-\frac{(4-x)^3}{x^2}  \right)=0$ and $\displaystyle -\frac{y}{f}+\lambda=0$. Substituting for $\lambda = y/f$ and simplifying produces $\displaystyle x+\frac{1}{2}\left( -\frac{3(4-x)^2}{x}-\frac{(4-x)^3}{x^2}  \right)=0$. Using Newton-Raphson to solve the equation gives $x = 2.309,\, y = 1.446$ The calculation is shown next using SymPy to do the differentiations
...
# ### Q90 answer
# When there are $n$ species, the sum of the mole fractions must still make unity making the constraint $\sum_{i=1}^n x_i = 1$ and the function is the entropy $S=-k_B\sum_{i=1}^nx_i\ln(x_i)$.
# 
# Using the Lagrange multiplier method for each species $i$ and by differentiating with respect to $x_1$ then $x_2$ etc., a series of $n$ equations is produced. The first two are $-k_B(\ln(x_1)+1)+\lambda=0$  and $-k_B  (\ln(x_2)+1)+\lambda = 0$. As all these equations are equal to one another, i.e. each are zero, it follows that $-k_B (\ln(x_1) + 1) = -k_B(\ln(x_2) + 1) = \cdots$ and so on for all species. As each $x_i$ is the mole fraction of species $i$, $x_i$ is a real number between 0 and 1 and with some simplifying it follows that $\ln(x_1) = \ln(x_2) = \cdots$ and hence,
# 
...
# 
# ### Q91 answer
# The minimisation function is $Q=-\sum_{v=0}^n x_v\ln(x_v)+\lambda\sum_{v=0}^n x_v=-\sum_{v=0}^n (x_v\ln(x_v)-\lambda x_v)$ with $\lambda$ as the undetermined multiplier, and the derivatives needed are $dQ/dx_v = 0$. Differentiating gives
# 
# $$\displaystyle \frac{d}{dx_v}\left( -\sum_{v=0}^n (x_v\ln(x_v)-\lambda x_v) \right)=-\sum_{v=0}^n(\ln(x_v)+1-\lambda=0$$
# 
# therefore, $\ln(x_v) + 1 - \lambda = 0$ or $x_v = e^{\lambda-1}$. Because the total of all the fractional populations $x_v$ is 1 then $\displaystyle \sum_{v=0}^n x_v = \sum_{v=0}^n e^{\lambda-1}= e^{\lambda-1}\sum_{v=0}^n 1=ne^{\lambda-1}=1$.
# 
...
# 
# ### Q92 answer
# Call the constraints $g(x,y,z)=x+y+z-1=0$ and $h(x,y,z)=x^2 +z^2 -1=0$. The $Q$ function is $Q = f + \lambda g + \mu h$ or explicitly $Q=4x+3y+10z+\lambda(x+y+z-1)+\mu(x^2 +z^2 -1)$ and their derivatives in $x$, $y$ and $z$ are set to zero.
