...
# 
# ![Drawing](fourier-fig18-top.png)
# ![Drawing](fourier-fig18-btm.png)
# 
...
# 
# **(1)** A finite wave train can be described as a sine wave $f(x) = \sin(\omega x)$ of  angular frequency $\omega = 2\pi/L$ that exists only over the range $-L$ to +$L$; this supposes also that the function $f(x)$ is zero everywhere else, see Fig. 19. Choosing the sine function to have the argument $2\pi x/L$ means that it is zero, i.e. has a node, at $x = \pm L$; note that the frequency need not be a multiple of the range of the transform, but the resulting equations are simpler if it is. Because the function is zero outside $\pm L$, so is the integral, and the integration limits of its fourier transform become $\pm L$ rather than $\pm \infty$. The forward fourier transform eqn. 27 gives,
# 
...
# 
# ![Drawing](fourier-fig19-top.png)
# ![Drawing](fourier-fig19-btm.png)
# 
...
# **(ii)** The fourier transform of an exponential decay $e^{-kt}$ has to be defined from zero as the decay has to start at $t=0$. The conjugate variable is $\omega$.  
# 
...
# **(i), (ii) Shift, delay.**
# 
# Let $s=x-x_0$ then $ds=dx$ and provided $f$ exists at $x_0$,
...
# 
# Make the substitution $s=ax,\; a>0$
# 
...
# **(iv) Reversal.** 
# 
# Start with scaling (as above (iii) ) and let $a=-1$ giving 
# $$\displaystyle F(f(-x)) = g(-k)$$
# 
# **(v) Complex conjugate.**
# 
# $g(-k)= \int_{-\infty}^\infty f(x)e^{ikx}dx$  taking the complex conjugate of both sides gives
# 
# $$\displaystyle g(-k)^*= \int_{-\infty}^\infty f(x)^*e^{-ikx}dx=f(x)^*$$
# 
# **(vi)(vii) Fourier derivatives.** 
# 
# These are important when solving differential equations. First derivative
# 
# $$\displaystyle \begin{align} 
# F\left(\frac{df}{dx}\right)  = \int_{-\infty}^\infty \frac{df}{dx}e^{-ikx}\, dx
# \end{align}$$
# 
# Integrate by parts and let $'u'=e^{-ikx}$ and $'v'=df/dx$ using $\int udv=uv-\int vdu$ gives
# 
# $$\begin{align} F\left(\frac{df}{dx}\right)&= \left |\frac{df}{dx}e^{-ik x}\right |^\infty_{-\infty} + ik \int_{-\infty}^\infty f(x)e^{-ik x} \,dx\\  & =0+ ik F\left( f(x)\right) \\ & =  ik g(k) \end{align}$$
# 
# By induction the $n^{th}$ derivative's transform is 
# 
# $$\displaystyle F\left(\frac{d^nf}{dx^n}\right)=(ik)^n g(k)$$
# 
# **(viii) Product.** 
# 
# The product $xf(x)$ can be useful in solving differential equations.
# 
