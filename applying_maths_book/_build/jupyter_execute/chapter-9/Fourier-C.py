...
# 
# In a fourier transform data represented as string of values taken at many sequential times, is transformed to allow the frequencies present be displayed and so analysed. The transform thus changes data expressed in time to data in frequency. In an nmr experiment the free induction decay (FID) is a complicated function that decays in time but the information we want to know about are the frequencies of the transitions between energy levels so that a molecule can be identified. This frequency information is obtained by fourier transforming the FID. Time and frequency form are said to form a conjugate pair. Anther conjugate pair is distance to reciprocal distance usually expressed in spectroscopy as wavelength to wavenumbers, i.e. to cm$^{-1}$.  More fundamentally, the instruments used to measure infrared and NMR spectra produce data that is itself the Fourier transform of the spectrum, and similarly, in X-ray crystallography, the image of spots produced on the detector is the Fourier transform of the planes of atoms in a crystal. 
# 
...
# 
# The effect of Fourier transforming a short and a long rectangular pulse is shown below, Fig. 15. The right-hand plots show the real part of the transform, which is a *sinc* (or Cardinal Sine) function, 
# 
# $$\mathrm{sinc}(ax) \equiv \sin(ax)/ax$$
# 
# The result of transforming is mathematically the same for both long and sort pulses, of course, but in a fixed frequency range the effect appears to be different. The shorter pulse has a wide central band set at zero and widely spaced side bands, which decay rapidly at frequencies away from zero and extend to infinity. The longer pulse has a narrower central band, also centred at zero, and higher frequency side bands than in the short pulse case; the results conform to $\Delta t\Delta v = 1$, i.e. short $\Delta t$ with wide $\Delta v$ and _vice versa_.
# 
# ### **(i) Finite length square pulse**
# 
...
# 
# In the last step the definition of the sine as an exponential is used, $\sin(x)=(e^{ix}-e^{-ix})/2i$. This function is plotted in figure 15 vs. frequency $k$. The function $\sin(ax)/(ax)\equiv \text{sinc}(ax)$ and occurs very often in fourier transforms and in this case it has the form,
# 
# $$\displaystyle  g(k) =\frac{a}{\sqrt{2\pi}}\frac{2}{ka}\sin\left(\frac{ka}{2}\right)$$
# 
# If the function depends on time then $x$ is replaced by $t$ and $k$ by the frequency $\omega$. The reciprocal nature of the function and its transform is also clear in these plots. The wider the function the narrower the transform and vice versa, this leads to an 'uncertainty principle' in which it is not possible to measure, at the same time, both the function and its transform with unlimited precision. This is described in detail later on. In Quantum mechanics this leads to the Heisenberg Uncertainty Principle. 
# 
# ![Drawing](fourier-fig15.png)
# 
# Figure 15. Example of the Fourier transform of a short and a long rectangular pulse each centred about zero and of total width $a$ and height $1$. Only the real part of the transform is shown, and is the sinc function, $\sin(ax)/ax$. The transform extends to $\pm \infty$ and $a=2$ in the top plots and $8$ in the lower ones. The transform crosses zero at equally spaced points which are $\pm n\pi/a$ where $n=1,2\cdots$.
# _______________
# 
# When the frequency $k=0$ the integral is 
# 
# $$\displaystyle g(0) = \frac{1}{\sqrt{2\pi}}\int_{-a/2}^{a/2} 1 dx =\frac{a}{\sqrt{2\pi}}$$
# 
# which is the size of the transform at zero frequency and the area under the function $f(x)$ divided by $\sqrt{2\pi}$ and this is true for any function $f(x)$.
# 
# ### **(ii) Finite Cosine and damped cosine**
# 
# What is the transform of a cosine wave of finite length? If the constant in the cosine is $k_0$, which has units of frequency, the transform is
# 
# $$\displaystyle g(k) = \frac{1}{\sqrt{2\pi}}\int_0^a \cos(k_0 x )e^{-ikx}dx $$
# 
# The real part of this integral is 
# 
# $$\displaystyle Re(g(k)=\frac{1}{(k^2-k_0^2)}(\sin(ak)\cos(ak_0)-\sin(ak_0)\cos(ak))$$
# 
# where the sine and cosine terms with argument $ak_0$ are constants as is $k_0^2$.  The result is shown in the top row of Fig. 16 and is somewhat similar to that of the square pulse except that the transform frequency cannot be centred at zero because the cosine has a finite frequency. The main peak is at the cosine's frequency, and many other sidebands are needed to account for the fact that the wave is suddenly turned off. The transform is in effect a *convolution* of a top hat or square function with a cosine wave. The longer the wave persists the narrower is the resulting transform as expected due to the reciprocal relationship between the conjugate variables, time and frequency.
# 
# Now suppose that the cosine is damped by an exponential function and smoothly decreases in amplitude, then these side-band frequencies largely disappear, because at the end of the cosine wave there is no discontinuity; the exponential makes the cosine gently approach zero. The result is a widening of the feature at the frequency of the cosine wave, Fig. 16. The effect of the exponential decay is to *apodise* the transform. This is shown in the lower panel of figure 16. The decay time of the exponential is not obvious in the transform as it is in the raw data. It manifests itself only in the width of the main frequency. Figure 17 shows the effect of lengthening the  pulse at the same frequency as figure 16 but now apodised with a longer lifetime decaying exponential $\tau=5$. The narrowing of the central peak is clear as now the outline shape of the pulse is longer, just as in figure 15.
# 
# ![Drawing](fourier-fig16A.png)
# 
# Figure 16. Top panel. Left: A truncated cosine wave of frequency $1$, starting at zero and of length of $6.5$ cycles. Right: The real part of its Fourier transform which shown many side bands due to the sudden termination of the signal and is reminiscent of that of a square wave. The value of the wave’s frequency is marked with a vertical line. Lower panel. Left the same wave apodised with $e^{-t/\tau}$ with $\tau=1$ which makes the cosine diminish to zero before it ends.
# _______________
# 
# ![Drawing](fourier-fig16b.png)
# 
# Figure 17. The same cosine wave as in fig 16 but extended for a longer time. Top Panel. Left the truncated wave and right its transform which is narrower in frequency than shown in fig 16, due to the reciprocal relationship between conjugate variables. Lower panel. The apodised wave and its transform. The apodising lifetime is now $\tau=5$ and because the signal lasts for longer this results in a narrower transform than in figure 16 even though $\tau$ is longer.  
# ________________
...
# 
# $$\displaystyle f(x)=\lim_{L \rightarrow \infty}\sum_{n=-\infty}^{\infty}\frac{L}{\pi}c_ne^{in\pi x/L}\Delta k =\int_{-\infty}^\infty c(k)e^{ikx} dk$$
# 
...
# check using sympy  1j is the definition of mathematical 'i' 

L, x, k = sp.symbols( 'L, x, k',positive =True)
pi  = sp.pi
f01 = sp.sin(2*pi*x/L)*sp.exp(-1j*k*x)

g = sp.simplify(sp.integrate(f01,(x,-L,L),conds='none') )
g
...
# 
# Figure 19a. Resonance lines with different Q. The peaks are normalised to 1 at $\omega_0$ but in reality the lower Q has a smaller area, i.e. produced from a cavity with less stored energy. The peak of the $Q=5$ signal is $25$ times larger than $Q=1$.
# ________
...
# The capital $F(\cdots)$ means 'do the transform'. The limits to the integral are $\pm \infty$.
# 
