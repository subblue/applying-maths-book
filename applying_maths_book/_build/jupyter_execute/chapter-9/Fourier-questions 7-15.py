...
# 
# 
# **Q8** If a rectangular light pulse of duration $t_p$ is used to excite a molecule, its electric field can be represented as $\displaystyle E(t) = e^{iω_0t}$, 0 $\lt t \lt t_p$.
# 
...
# 
# 
# **Q9** In an NMR experiment, nuclear spins are excited by a rectangular shaped electrical pulse of length $\tau$
# and amplitude $a$.
# 
...
# 
# **Q11** **(a)** Show that, if the phase $\displaystyle \varphi$ of $\sin(2\pi t/t_0 + \varphi)$ is chosen not to be $n\pi$, where $n$ is an integer including zero, then both real and imaginary parts of its Fourier transform exist.
# 
# >**(b)** When the phase is n$\pi$ show that the transform is imaginary.
# 
# **Q12** Suppose that the electron density of an atom can be represented in one dimension as a delta function at a distance $a$ from the origin; $\delta(x - a)$. The scattering of electromagnetic waves, such as X-rays off an atom's electrons is represented by the Fourier transform with such a delta function.
# 
...
# 
# **Q13** Following on from the previous question, suppose now that there are two (atom) delta functions separated by a distance $2a$. Show that the Fourier transform is a cosine and calculate the probability density.
# 
...
# 
# 
# **Q15** A laser produces an exponentially shaped pulse $\displaystyle p = e^{-|\,ax\,|}$, with a duration (fwhm) of $\tau = 10$ fs at 800 nm. 
# 
# >**(a)** Calculate _a_, then calculate the frequency spread of the pulse and 
# 
