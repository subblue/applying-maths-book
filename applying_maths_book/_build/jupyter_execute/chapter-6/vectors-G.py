...
# 
# $$\displaystyle \vec T = \vec r \times\vec F$$
# 
...
# 
# $$\displaystyle W=\mu E\int_{\pi/2}^\theta \sin(\theta)d\theta=-\mu E\cos(\theta)$$
# 
# If the dipole and field are made into vectors the energy is $W = -\vec \mu\cdot \vec E$. A similar calculation
# would apply if a magnetic dipole were placed in a magnetic field.
# 
# ![Drawing](vectors-fig52.png)
# 
...
# 
# $$\displaystyle \vec L= \vec r \times m\vec v=m\vec r\times(\vec \omega \times \vec r) $$
# 
...
# 
# $$\displaystyle \begin{align} \vec L=&m\vec r\times(\vec \omega \times \vec r)\\=&m(\vec r\cdot\vec r)\vec \omega -m(\vec r \cdot \vec \omega)\vec r\\=&m|\vec r|^2\vec\omega-m|\vec r|\omega|\cos(\theta)\vec r\end{align} $$
# 
...
# 
# $$\displaystyle |\vec L | = L = mr^2|\vec \omega | = mvr$$
# 
...
# 
# $$\displaystyle E = -\gamma IB = \pm \frac{1}{2} \hbar \gamma B_0 $$
# 
