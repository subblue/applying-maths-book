...
# Mg atoms. Then write down the rate constant matrix for the transfer from any one molecule to each of the others. This is a $7\times 7$ matrix and every element will be filled. Because all the molecules are the same, the transfer rate constant from molecule 1 to 2 is the same as the reverse 2 to 1; there is no Boltzmann factor to take into account and therefore the matrix is symmetrical. Assume that the BChl molecules decay with a lifetime of $5$ ns so that the fluorescence rate constant is $k_f = 1/5 \times 10^9\;\mathrm{ s^{-1}}$ and $R_0 =70\; \overset{\mathrm{o}}A$.
# 
