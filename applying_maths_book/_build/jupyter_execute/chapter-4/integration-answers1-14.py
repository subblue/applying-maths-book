...
# 
# ### Q2 answer
# (a) Taking logs of both sides of the Arrhenius equation produces $\displaystyle \ln(k) = \ln(k_0) - \frac{E_a}{RT}$. Differentiating with respect to $T$ gives $\displaystyle \frac{d\ln(k)}{dT}=\frac{E_a}{RT^2}$ as in the question.
# 
# **(b)** To integrate this differential equation, separate the equation into parts in $k$ and in $T$ as $\displaystyle \int_{k_1}^{k_2}d\ln(k) = \int_{T_1}^{T_2} \frac{E_a}{RT^2}dT$ and integrate both sides separately. Because the term $d\ln(k)$ integrates to $\ln(k)$ the result is 
# 
...
# 
# ### Q3 answer
# Rearranging to separate variables produces $\displaystyle \int\frac{dc}{c}=-k\int dt$. Integrating both  sides separately gives $\ln(c)=-kt+q$ where $q$ is a constant. 
# 
...
# 
# First taking the result $\ln(c) = -kt + q$, when $t = 0,\, c = c_0$ then $q = \ln(c_0)$ and therefore $\ln(c) = -kt + \ln(c_0)$ or $\displaystyle\ln\left((\frac{c}{c_0}\right)=-kt$ and secondly by adding the limits in the integration initially as 
# 
...
# 
# ### Q4 answer
# (a) Rearranging the equation and adding limits gives $\displaystyle \int_{v_0}^v\frac{dv}{v}=-\frac{3\pi\delta\eta}{m}\int_0^t dt$ and integrating gives $\displaystyle \ln(v)\bigg|_{v_0}^v=-\frac{3\pi\delta\eta}{m}t\bigg|_0^t$ whcih can be rearranged to give
# 
# $$v=v_0\exp\left(-\frac{3\pi\delta\eta }{m}t\right)$$
# 
# The exponential must be dimensionless therefore the ratio, $\displaystyle \tau = \frac{m}{3\pi\delta\eta}$ represents a time. To prove that this is a time, add dimensions to the constants. Viscosity is usually measured in centipoise which is not an SI unit. The SI units of viscosity are $\mathrm{ kg\, m^{-1} \,s^{-1}}$ and $1 cP = 0.001\,\mathrm{ kg\, m^{-1}\, s^{-1} }$. The dimensions are then $\displaystyle \frac{m(\mathrm{kg}) }{\delta(\mathrm{m}) \eta(\mathrm{ kg\, m^{-1}\, s^{-1} })  }\equiv s$
# 
...
# \rho\delta^3$. The time is therefore $\displaystyle \tau =\frac{\rho\delta^3  }{3\pi\eta}\approx 10^{-12}\text{s}$ which means that the direction of the initial movement is lost within about a picosecond, which is a very short time considering the relatively large size of a protein.
# 
# ### Q5 answer
...
# 
# (b) Converting to exponentials gives $\displaystyle I=\int\cosh^2(x)dx=\frac{1}{4}\int\left(e^{2x}+2+e^{-2x}\right)dx =\frac{1}{8}\left(e^{2x}+4x-e^{-2x} \right)+c$. With some manipulation this can be returned to a trig form which is $x/2 + \sinh(2x)/4$.
# 
# (c) This can be converted to exponentials first giving $I=\cosh^2(x)/2$
# 
# (d) Expanding the integral gives $I=\int 2\ln(2x)-\ln(1-x)dx$ which has a standard form, see 2.13.
# 
# (e) The function in the integral is odd so the integral is zero because the limits are symmetrical about zero.
# 
# (f) The integral becomes a standard one the type $x^{n+1}/(n+1)$ by letting $u-x-3$. Thus this is an example of using substitution to simplify and then solve the integral. $\displaystyle I=\int u^{1/2}du =\frac{2}{3}u^{3/2} \to \frac{2}{3}(x-3)^{3/2}$.
# 
# Adding limits produces $\displaystyle I=\frac{2}{3}(x-3)^{3/2}\bigg|_{-1}^1=\frac{2}{3}\left((-2)^{3/2}-(-4)^{3/2}\right)=\frac{4i}{3}\left(\sqrt{2}-4\right)$.
# 
# ### Q6 answer
# Converting to the exponential form the integral is  $\displaystyle \int_0^L \frac{1}{4}\left( 2-e^{2iLx}-e^{-2iLx} \right)dx = \frac{L}{2} -\frac{\sin(2L^2)}{4L} $.
# 
...
# 
# ### Q10 answer
# (a) If $V_0$ is the initial, $V_1$ the final volume then the work done is $\displaystyle w=-\int_{V_0}^{V_1}pdV$ where $\displaystyle p=\frac{nRT}{V-nb}-\frac{an^2}{V^2}$. Integrating gives
# 
...
# 
# ### Q12 answer
# (a) The Clapeyron equation for a change of phase such as melting or evaporation is $\displaystyle \frac{dp}{dT}=\frac{\Delta S}{\Delta V}$. Because $ \Delta S=\Delta H/T$ for the phase change, $\displaystyle \frac{dp}{dT}=\frac{\Delta H}{T\Delta V}$. For a liquid to vapour transition this is written as $\displaystyle \frac{dp}{dT}=\frac{\Delta_{vap} H}{T\Delta_{vap} V}$.
# 
# Separating terms in pressure and temperature produces $\displaystyle \int_{p_1}^{p_2}dp= \frac{\Delta H}{\Delta V}\int_{T_1}^{T_2} \frac{dT}{T}$ and therefore 
# 
...
# 
# (b) The Clausius-Clapeyron equation is obtained by allowing the change in volume (per mole) on forming the vapour, to be far larger than that of the original liquid making $\Delta_{vap}V \approx V_{vap}$ and therefore $\Delta_{vap}V=RT/p$ and $\displaystyle \frac{dp}{dT}=p\frac{\Delta_{vap}H}{RT^2}$.  This is the Clausius-Clapyron equation equation that is usually written as
# 
# $$\frac{d\ln(p)}{dT}=\frac{\Delta_{vap}H}{RT^2}$$
# 
# Integrating $\displaystyle \int_{p_1}^{p_2}d\ln(p)= \frac{\Delta_{vap} H}{R}\int_{T_1}^{T_2} \frac{dT}{T^2}$ produces  $\displaystyle \ln\left(\frac{p_2}{p_1} \right) = -\frac{\Delta_{vap}H}{R}\left( \frac{1}{T_2}-\frac{1}{T_1} \right) $
# 
# ### Q13 answer
...
# 
# The Clausius-Clapeyron equation describes solid - vapour and liquid - vapour changes because the final volume is far
# greater than the initial one, and is $\displaystyle \frac{dp}{dT}=p\frac{\Delta H}{RT^2}$ where $\Delta H$ the enthalpy change at the liquid – vapour or sublimation transition. Integrating this last equation from pressure p1 to p2 and temperature $T_1 \to T_2$ gives $\displaystyle \ln\left(\frac{p_2}{p_1} \right) = -\frac{\Delta_{vap}H}{R}\left( \frac{1}{T_2}-\frac{1}{T_1} \right) $ as discovered in the previous question.
# 
...
# 
# ### Q14 answer
# (a) $\displaystyle \left( \frac{\partial q}{\partial p} \right)_S =V$ integrates, at constant entropy, to become $\displaystyle \int_{q_0}^{q}dq=\int_{p_0}^pVdp=\int_{p_0}^p\left( \frac{C}{p} \right)^{1/\gamma}dp$. This integration is of the $x^{n+1}/(n + 1)$ form, equation 6, therefore,
# 
...
# This equation must be zero at the minimum nozzle diameter and can be solved by factoring the bracket in the numerator to give the pressure ratio in the throat of  $\displaystyle \frac{p}{p_0}=\left( \frac{2}{\gamma+1} \right)^{\gamma/(\gamma-1)}$  which, somewhat surprisingly, depends only on the ratio of heat capacities $\gamma$. The gas velocity in the throat is obtained by substituting this pressure into the equation for the speed, giving $\displaystyle U_s=\sqrt{\frac{2\gamma}{\gamma+1}\frac{RT_0}{m}}$  which is the speed of sound in the gas under the prevailing conditions. 
# 
