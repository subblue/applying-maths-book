...
# \end{array}$$
# 
# Although we have calculated the average energy, the average displacement and its square are just as easily calculated. This is done by adding the lines $\mathtt{ xtot=xtot+x1}$ and $\mathtt{x2tot=x2tot+x1^{**}2}$ after the lines calculating the energy, and by defining these values to be zero initially. The calculated values are shown in the table and are  are close to the theoretical values. In the algorithm, and only for clarity, adding E1 or x1 etc to their respective totals starts immediately, without allowing the random walk to come close to its equilibrium value, which takes several hundred steps. This introduces an error but as the number of Monte-Carlo steps is large, $20000$, this error is small.
# 
# Estimations of the range of the average potential energy of a harmonic oscillator using the Metropolis method are shown in figure 12a. The number of samples is the same in each case. For example, one run of $10^5$ samples or 1000 runs of 100 samples. It is clear that sampling with a longer random walk reduces the standard deviation.
# 
# ![Drawing](monte-carlo-fig12a.png)
# 
# Figure 12a : Metropolis estimation of potential energy vs. number of samples. The estimated energy is plotted as red circles. The horizontal line is the theoretical energy $k_BT/2$ at 300 K. The solid lines are given by $\langle E\rangle \pm a /\sqrt{ N}$ where $a=2.5$ is a constant.
# _____
# 
...
# The results are shown in figure 12b, where the Metropolis data from $1000$ samples is shown as red dots on top of the known distribution eqn 13c (wide grey line) using a mass of $14$ amu at $300$ K.  It is clear that the simulation matches the data remarkably well.
# 
# ![Drawing](monte-carlo-fig12b.png)
# 
# Figure 12b. Maxwell speed distribution showing the distribution of sampled points. The thick grey curve is the theoretical line.
# _________
# 
...
# 
# ![Drawing](monte-carlo-fig12d.png)
# 
# Figure 12e. A schematic of two configurations of spins related by a single spin flip. Only values of $\Delta E = 0, \pm 4J$ occur. The interaction energy is $-J$ _between a pair of parallel spins_ whether up or down, and $+J$ if anti-parallel. The energy change is calculated as final - initial state.
# ____
# 
# The code to calculate 1D Ising spin energy and heat capacity as in  figure 12f is shown below. The parameters to be used are defined outside the routine Ising1D but can be used inside as in Python they are global variables.
...
# ![Drawing](monte-carlo-fig12f.png)
# 
...
# 
# The entropy of the equilibrium state is given by $S = k_B \ln(\Omega)$ where $\Omega$ is the number of arrangements that the spins can take. To calculate $\Omega$, the number of groups of spins p present at the end of the calculation and at a given temperature is counted; see Figure 12g. This is done by searching the pattern of spins for the number of spin changes and adding one to the total. From the theory of statistics, the number of ways of choosing $N_p$ distinguishable objects from a group of $N$ distinguishable objects, where the order of choosing does not matter, is given by
# 
# $$\displaystyle \Omega ' = \frac{N!}{N_p!(N-N_p)!}  \tag{13e} $$
# 
# This is the number of arrangements, or combinations and also the binomial coefficients. Equation 13e is a good approximation to the number of arrangements of groups of spins, when $N$ is large and the temperature is not zero, thus $\Omega \approx \Omega '$. The good agreement, Figure 12f, between the equation describing the entropy per spin and that obtained from the spin state on using the Metropolis algorithm shows that this is a good approximation.
# 
...
# 
# Within statistical variation, the entropy per spin obtained by counting groups of spins and using eq 13e and that calculated from theory are the same, see Figure 12g.
# 
# ![Drawing](monte-carlo-fig12g.png)
# 
...
# 
# ![Drawing](monte-carlo-fig12i.png)
# 
...
# 
# Figure 13. Two patterns of spins in the Ising model. The dotted line shows the near neighbours to the central spin, which is flipped between the two configurations.
# ____
# 
...
# 
# One run of the algorithm with $25000$ samples per spin, or $900000$ samples in total on a six by six lattice, produces the result $E = -1.75 \pm 0.3$ and $C_V = 0.686$ with $T = 2$. Figure 14 shows $C_V$ and the magnetization per spin on a two-dimensional lattice of sixteen by sixteen elements at $25000$ samples / spin at several values of $k_BT/J$ together with the theoretical result 
# 
# $$\displaystyle M_{Th} = (1 - \sinh^{-4}(2J/k_BT))^{1/8}$$
# 
# ![Drawing](monte-carlo-fig14.png)
# 
# Figure 14 Ising model calculations using the Metropolis method on a 16 by 16 lattice. Left: Heat capacity vs temperature as $k_BT$. Right: Theoretical magnetization (solid line) for an infinite lattice and calculated points for the sixteen by sixteen lattice.
# ____
