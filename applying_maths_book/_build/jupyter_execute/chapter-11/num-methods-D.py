...
# 
# ### 7.6 Sensitivity to rate constants
# 
# To understand some of the equations above, such as the maximum fraction infected, $I_N^{max}$ it is easier to draw some graphs. Suppose that there are ten thousand susceptible persons, and that the disease starts to spread after one person is infected. What is the effect of the different rate constants? The rate of infection $k_2$ here is key, i.e. the term $k_2SI$ (eqn. 41) has a dramatic effect. If $k_2$ is small, $10^{-5}$, see the graphs below, the overall the epidemic does not progress even up to $1000$ days. However, with $k_2=2\cdot 10^{-5}$ the epidemic starts and $\approx 1500$ persons are infected at the peak. Increasing $k_2$ again to $5\cdot 10^{-5}$  $\approx 4800$ are infected. This rapid increase is rather dramatic and shows the effect of feedback, i.e. $ S+I \overset{k_2}\longrightarrow 2I $;  Increasing $k_2$ above $10^{-4}$ has a smaller effect than when $k_2=10^{-5}$ simply because so many are already infected.
# 
# This behaviour shows how important the reproductive ratio is as this will be proportional to $k_2$. Even with this simple model we can also understand why wearing face masks is important because this effectively reduces $k_2$ and isolating individuals, or being vaccinated, reduces the spread of disease simply because the number of those susceptible is reduced and so is the product $k_2SI$ in eqn. 40. 
# 
# ![Drawing](num-methods-fig16a.png)
# 
# Figure 16a. Populations of susceptible (S), infected (I) and recovered (R) persons vs. time with different $k_2$ values using $k_1 =0.1$. The peaks of the infected curves are given using eqn 43, $I_N^{max}N$.
# _________
# 
