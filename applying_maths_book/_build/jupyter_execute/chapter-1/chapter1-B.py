...
# 
# The azimuthal angle $\phi$ is in the $x-y$ plane and measures the angle from the x-axis to the line formed by the projection of the point onto this plane, and has a maximum of 360$^\text{o}$. $\theta$ is the polar angle measured from 0 to 180$^\text{o}$ from the z-axis. Sometimes when doing an integration it is advantageous to change coordinates from $xyz$ to spherical polar ones. The volume element $dxdydx$ then becomes $r^2\sin(\theta)drd\theta d\phi$ and the limits on the integration are also changed to $0\to 2\pi$ for $\phi$, $0\to \pi$ for $\theta$ and $0\to \infty$ for $r$. How the volume element is formed is shown using Jacobian determinants in Chapter 4, section 10.11. 
# 
...
# 
# The angle $\theta$ is measured from the x- to the y-axis, figure 18. The volume element used in integrations is changed as  $dxdy \to rdrd\theta$ and the integration limits fro $\pm\infty$ to $0\to \infty$ in $r$  and $0\to 2\pi$ in $\theta$. (Note that $\theta$ is defined differently than that in spherical polar coordinates).
# 
