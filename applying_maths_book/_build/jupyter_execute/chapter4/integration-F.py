...
# 
# $$\displaystyle  \langle x\rangle =\frac{1}{A}\int\int xf(x,y)\,dxdy,\qquad \langle y\rangle =\frac{1}{A}\int\int yf(x,y)\,dxdy,\qquad A=\int\int f(x,y)\,dxdy \tag{54}$$
# 
...
# 
# The y-direction moment is calculated similarly using $x^2$, $\displaystyle I_y=\int\int x^2f(x,y)dxdy $. If the mass is uniform then the function f is a constant and can be taken outside the integration.
# 
...
# The angular part of the integral $\displaystyle \int_0^{2\pi}d\phi\int_0^\pi \sin(\theta)e^{i\phi} \sin(\theta)e^{-i\phi} \sin(\theta) d\theta d\phi$ are simplified by separating the $\theta$ and $\phi$ integrals into two then evaluating the complex conjugate first, because $e^{-i\phi}e^{i\phi} = 1$. The remaining integral is
# 
...
# can be written as
# 
# $$\displaystyle \int f(x)dx= \int F(u)\frac{dx}{du}=\int F(u)\,J(x,u)du$$ where $F$ is the function $f$ in the new variable $u$ and the new function $J$ contains the terms needed to 'distort' $dx$ into $du$. This is done by using the differential $dx = J(x, u)du$  where $\displaystyle J(x,u)=\frac{dx}{du}=-\frac{1}{2x}=-\frac{2}{2\sqrt{a^2-u}}$
# 
...
# 
# **(a)** Calculating the Jacobian,<br>
# **(b)** Substituting the new variables into the function $f$,<br>
...
# 
# $$\displaystyle J\left( \frac{x,y,z}{r,\theta,\varphi} \right) \equiv \frac{\partial(x,y,z)}{\partial(r,\theta,\varphi)} = 
# \begin{vmatrix}
...
# 
# #### example (i)
# The complicated integral $\displaystyle \int\int \frac{1}{\sqrt{(x-y)^2+2(x+y)+1}}dxdy$ can be simplified markedly with the coordinate change $x=u(1+v),\, y=v(1+u)$. The general form of the equation is $\displaystyle \int\int f(x,y)dxdy=\int\int F(u,v)\frac{\partial(x,y)}{\partial (u,v)}dudv$. The first two steps in the calculation are necessary because no limits are given. In step (a) the Jacobian is calculated and is
# 
...
# 
# #### example (ii)
# The integral $\displaystyle I=\int_0^1\int_0^\sqrt{1-x^2} x(1-xy^3)dydx$ can be solved by converting to (plane) polar coordinates. The limits are converted first. As $x$ extends from $0 \to 1$ so does $r$ as this is the radius in polar coordinates. As $x = r\cos(\theta)$ and $y = r\sin(\theta)$ then $r^2 = x^2 + y^2$, which represents a circle. The maximum value $r = 1$; therefore $x^2 + y^2 = 1$ and integration is around the first quadrant of a circle of unit radius, in the new coordinates the integration area is a rectangle where $r$ ranges from $0 \to 1$ and $\theta$ from $0 \to \pi/2$. The angle $\theta$ varies from $0 \to \pi/2$ and $r$ from $0 \to 1$; Figure 26.
# 
# The integral is 
# 
...
# ______
# 
# Doing the integral in $\theta$ first produces $\displaystyle r^2-\frac{2}{15}r^6$ which is easily integrated to five $\displaystyle I=\frac{11}{35}$. The angular integral using Sympy is
...
# #### (iii)
# In this example the integral $\displaystyle I= \int_0^\infty\int_0^x x^2e^{-x^2-y^2}dxdy$ will be solved by transforming to polar coordinates. Using equation 59 gives
# 
...
# 
# Substituting into the integral gives $\displaystyle \int_0^\infty \int_0^x x^2e^{-x^2-y^2}dxdy =\int_0^\infty \int_0^{\pi/4} r^2\cos^2(\theta)e^{-r^2}rdrd\theta$ which can be separated into integrals in $r$ and $\theta$, since there is no term in both variables, and the limits of the integration are constants. The integrals are standard ones, see Section 2.13. Using Sympy gives $\displaystyle I=\frac{1}{8}\left( \frac{\pi}{2}-1  \right)$. The calculation is shown below without limits.
...
# _____
# #### example (iv)
# The complicated integral $\displaystyle I = \int_0^3\int_0^1\frac{x^2}{(x^2+y^2)^{5/2}}dxdy$ can be solved by converting to polar coordinates first then changing the integration limits. Converting produces  $\displaystyle I = \int\int\frac{\cos^2(\theta)}{r^2}dr d\theta$.
# 
...
# 
# The integration has to be split into two parts because CD is sloping: the areas ABCE and CDE. The coordinates of the points are the intersections of their respective curves, except E, which is at point $(\tan^{-1}(3), 1/\sin(\tan^{-1}(3)) )$. The integration limits for $r$ are $1/\sin(\theta) \to 3/\sin(\theta)$ and for area ABCE, $\theta = \tan^{-1}(3) \to \pi/2$ and for CDE, $\theta = \pi/4 \to \tan^{-1}(3)$. 
# 
# The calculation in Sympy is shown below and is in two parts The result for the first part is $\displaystyle \int_{\pi/4}^{\tan^{-1}(3)} \int_{1/\sin(\theta)}^{1/\cos(\theta)} \frac{\cos^2(\theta)}{r^2} dr d\theta =\frac{1}{\sqrt{2}}-\frac{16\sqrt{10}}{75}$
