...
# 
# The constant appears because no upper or lower limit on the volume was defined and so the value of the constant is unknown. Mathematically this equation is fine as it stands, but to add real numbers the log has to be dimensionless, however, volume has dimensions of m$^3$ therefore $V$ in the equation is made dimensionless by  simply taking its numerical value. Doing this is not strictly correct but as the constant is arbitrary until some values are added, the equation can be rewritten as 
# 
# $$\displaystyle w_{rev} = -nRT \ln(V/V_c)$$
# 
# where the constant is now changed to $V_c$ and incorporated into the log. $V_c$ still does not have a value, that will depend on the actual case being calculated, but now has units of volume, which reminds us that the log is dimensionless.
# 
# In the case where the gas is not ideal the pressure will be given by some other function. One such was devised by van der Waals and has the form  $\displaystyle \left(p+\frac{an^2}{V^2}\right)\left(V-b\right)=nRT $ where $a$ and $b$ are constants depending on the particular gas. 
# 
...
# #### Summary
# _____
# >If $\displaystyle I=\int_{-a}^a f(x)dx$, and $f(x)$ is odd, $f(-x) = -f(x)$ and $I = 0$,
# 
# >however, if $f(x)$ is even, $f(-x) = f(x)$, then $I \ne 0$.
# _____
# 
# The left graph, figure 4, shows an odd function, $x^3 - x$. There are many others; for example, $\sin(x)$ over the range $\pi \to \pi$, where the area from zero to $-x$ is equal but opposite to that from $0 \to x$ making zero in total. The integral $\displaystyle \int_{-a}^a (x^3-x)dx =\frac{x^4}{4}-\frac{x^2}{2}\bigg|_{-a}^a=0$ and this is because because $a^4 = (-a)^4$ and $a^2 = (-a)^2$. The right-hand graph shows an even function where the area is not zero but negative.
# 
...
# 
# ### 2.2 $\int dx$, $\int d\ln(x)$ and similar expressions
# 
...
# _______
# 
# ### 2.7 Exponential form of trig functions
# 
...
# 
# A number of functions when integrated produce infinity. These are conventionally called improper integrals, although there is nothing 'improper' about them, e.g. $\displaystyle \int_0^\infty \frac{e^x}{x}dx=\infty$.  The integrand $f(x)$ usually has a point in the range of integration at which it becomes infinite. This can often be spotted, and surprisingly, only sometimes does this produce an infinite result. As an example, in the integral $\displaystyle \int_0^\infty \frac{1}{\sqrt{x}}dx$ the reciprocal of the square root becomes infinite at $x = 0$. Ignoring this for the moment, integrating using equation 6 then working out the limits produces a finite answer, $\displaystyle \int_0^\infty \frac{1}{\sqrt{x}}dx=2\sqrt{x}\bigg|_0^1=2$. This is unexpected because the function is infinite at exactly $x=0$ so one would expect an infinite area from 0 to 1 but the area  is actually finite.
# 
# Conversely the integrals of the form  $\displaystyle \int_0^\infty \frac{1}{x^2}dx$ or $\displaystyle \int_0^\infty \frac{1}{x-1}dx$ and many similar reciprocal functions produce infinity and so are undefined or improper. Infinity is s strange beast.  Not surprisingly, if the range of the integration is changed so that the integrand is not infinite in the range, the integrals behave normally and give a finite result.
# 
# 
...
# 
# Very occasionally a function has a discontinuity such as a vertical step. If this is at point b and the integration is from a to c with b in between, the integral is split into two parts, one part from $a \to b$ the other from $b \to c$; see Section 1.3.
# 
# 
# ### 2.13 A few common integrals using SymPy
# 
# Many integrals can be obtained using SymPy or other algebraic engines such as Maple of Mathematics.
# 
...
# 
# $\displaystyle \int_a^b \frac{1}{a x}  =  \frac{\log{\left (x \right )}}{a} $
# 
# $\displaystyle \int_a^b e^{a x + b}  =  \frac{e^{a x + b}}{a} $
# 
# $\displaystyle \int_a^b \frac{1}{a x + b}  =  \frac{\log{\left (a x + b \right )}}{a} $
# 
# $\displaystyle \int_a^b \sin{\left (a x + b \right )}  =  - \frac{\cos{\left (a x + b \right )}}{a} $
# 
# $\displaystyle \int_a^b \cos{\left (a x + b \right )}  =  \frac{\sin{\left (a x + b \right )}}{a} $
# 
# $\displaystyle \int_a^b \tan{\left (a x + b \right )}  =  \frac{\log{\left (\tan^{2}{\left (a x + b \right )} + 1 \right )}}{2 a} $
# 
# $\displaystyle \int_a^b \sinh{\left (a x + b \right )}  =  \frac{\cosh{\left (a x + b \right )}}{a} $
# 
# $\displaystyle \int_a^b \cosh{\left (a x + b \right )}  =  \frac{\sinh{\left (a x + b \right )}}{a} $
# 
# $\displaystyle \int_a^b \tanh{\left (a x + b \right )}  =  x - \frac{\log{\left (\tanh{\left (a x + b \right )} + 1 \right )}}{a} $
# 
# $\displaystyle \int_a^b \log{\left (a x + b \right )}  =  x \log{\left (a x + b \right )} - x + \frac{b \log{\left (a x + b \right )}}{a} $
# 
# $\displaystyle \int_a^b \frac{1}{\left(a x + b\right)^{2}}  =  - \frac{1}{a \left(a x + b\right)} $
# 
# $\displaystyle \int_a^b \frac{1}{a^{2} x^{2} + b^{2}}  =  \frac{i \left(- \log{\left (x - \frac{i b}{a} \right )} + \log{\left (x + \frac{i b}{a} \right )}\right)}{2 a b} $
# 
# $\displaystyle \int_a^b \frac{1}{\sqrt{a^{2} x^{2} + b^{2}}}  =  \frac{\operatorname{asinh}{\left (\frac{x \left|{a}\right|}{b} \right )}}{\left|{a}\right|} $
# 
# $\displaystyle \int_a^b \frac{1}{a^{2} x^{2} - b^{2}}  =  \frac{\log{\left (x - \frac{b}{a} \right )} - \log{\left (x + \frac{b}{a} \right )}}{2 a b} $
# 
# $\displaystyle \int_a^b \frac{1}{\sqrt{a^{2} x^{2} - b^{2}}}  =  \begin{cases} \frac{\operatorname{acosh}{\left (\frac{x \left|{a}\right|}{b} \right )}}{\left|{a}\right|} & \text{for}\: a^{2} \left|{\frac{x^{2}}{b^{2}}}\right| > 1 \\- \frac{i \operatorname{asin}{\left (\frac{x \left|{a}\right|}{b} \right )}}{\left|{a}\right|} & \text{otherwise} \end{cases} $
# 
# $\displaystyle \int_a^b \sqrt{a x + b}  =  \frac{2 \left(a x + b\right)^{\frac{3}{2}}}{3 a} $
# 
# $\displaystyle \int_a^b \frac{1}{\sqrt{a x + b}}  =  \frac{2 \sqrt{a x + b}}{a} $
# 
# $\displaystyle \int_a^b x e^{- a x}  =  - \frac{\left(a x + 1\right) e^{- a x}}{a^{2}} $
# 
# $\displaystyle \int_a^b x^{2} e^{- a x}  =  - \frac{\left(a^{2} x^{2} + 2 a x + 2\right) e^{- a x}}{a^{3}} $
# 
# $\displaystyle \int_a^b e^{- a x^{2}}  =  \frac{\sqrt{\pi} \operatorname{erf}{\left (\sqrt{a} x \right )}}{2 \sqrt{a}} $
# 
# $\displaystyle \int_a^b x^{2} e^{- a x^{2}}  =  - \frac{x e^{- a x^{2}}}{2 a} + \frac{\sqrt{\pi} \operatorname{erf}{\left (\sqrt{a} x \right )}}{4 a^{\frac{3}{2}}} $
# 
# $\displaystyle \int_a^b x^{2} e^{- a x^{2}}  =  - \frac{x e^{- a x^{2}}}{2 a} + \frac{\sqrt{\pi} \operatorname{erf}{\left (\sqrt{a} x \right )}}{4 a^{\frac{3}{2}}} $
# 
...
for  i, eqn in enumerate(eqns): 
    print(simplify(integrate(eqn, x) ),end=',  ')
    print('$\displaystyle \int_a^b',latex(eqn),
          ' = ',latex( simplify(integrate(eqn, x) ) ) , '$\n' ) # show as markdown
