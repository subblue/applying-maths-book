...
# 
# #### Chain reactions. Mean Chain length.
# 
...
# 
# figure 17c: Semenov's scheme for a simple chain reaction.
# ____
# 
...
# \frac{d[A]}{dt}&= I- (v_1+v_2)[A]+v_1[A] \\&= I-\frac{(1-\alpha)}{\alpha}v_1[A] \tag{43a}\\
...
# 
# Figure 16. left: A constant or uniform distribution. Right: A Gaussian distribution. These distributions are not normalised as their area is not unity.
# ____
# 
# #### (i) Excited states.
# 
...
# 
# In this example $\alpha = 1/\tau$ and $x=t$ thus the integral is $\int_0^\infty e^{-t\tau}dt = \tau$. Then $\displaystyle \frac{d}{d(1/\tau)}\ln(\tau)=\tau$ which is the same result as direct integration. Note that the differentiation is wrt. $\alpha$ but in this case $\alpha =1/\tau$ so differentiation is in $1/\tau$. 
# 
# #### (ii) Infection period.
# 
...
# 
# ![Drawing](integration-fig17.png ) ![Drawing](integration-fig17b.png )
...
# 
# where $N$ is the number of theoretical plates, $p$ the probability related to the partition constant $K$  
# 
# $$\displaystyle p=\left(1+K\frac{V_s}{V_m}\right)^{-1}$$
# 
# total volume of the mobile phase $V_m$ and of $V_s$ the stationary phase. Initially $m_0$ moles are added and are distributed in the mobile and stationary phase as $p:(1-p)$. The variable $n$ is the number of steps takes as the molecules move down the column and at each step $V_m/N$ moles are elute.
# 
...
# 
# We define the uncertainty in the measurement of a quantity $x$ as its root mean square value or the square root of the variance. The variance is  $\displaystyle \sigma^2 =\sum_i{(x_i-\langle x\rangle )^2 }/(n-1)$, see chapter 13 on Data Analysis. The standard deviation is $\sigma$. The summation indicates an average of the $x_i$ is being taken and expanding out this equation it can be re-written as 
# 
