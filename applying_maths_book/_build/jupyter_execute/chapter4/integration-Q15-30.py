...
# \text{(a) } I=\int\cos^3(x)dx & \text{(b) }\displaystyle I=\int \frac{3x}{(5+3x)^4}dx &\qquad    &\qquad       &\qquad\qquad&\qquad\qquad&\qquad\qquad\\
# \text{(c) } \displaystyle I=\int \frac{e^\sqrt{x}}{\sqrt{x}}dx &\text{(d) }\int \cot(ax)dx\\
# \text{(e) }\displaystyle I=\int\frac{x^2}{8+x^3}dx &\text{(f) }\displaystyle \int \frac{e^{ax}}{1-e^{ax}}\\
# \end{array}$$
# 
# ### Q16
# Find $\displaystyle I=\int \frac{1}{\sqrt{b^2-x^2}}dx$.
# 
# **Strategy:** You might well decide at this point to try using Sympy as this integration is looking quite complicated and the substitution to use is not obvious. Evaluation can sometimes be successful by starting with $b^2\cos^2(u) + b^2\sin^2(u) = b^2$ or $b\cos(u) =\sqrt{ b^2 - b^2\sin^2(u)}$. Make the substitution $x = b \sin(u)$ and its differential $dx = b \cos(u)du$.
# 
# ### Q17
# (a) Integrate $I=xe^{-x^2}dx$ and
# 
...
# 
# ### Q21
# The integral $\displaystyle Ei(x) = -\int_{-x}^\infty \frac{e^{-t}}{t}dt$ is called the _exponential integral_ and cannot be evaluated explicitly but only numerically. It can occur in the calculation of molecular orbitals.
# 
...
# 
# (b) Sometimes integrals can be written in terms of others. Suppose that $I_0=\int e^{ax}dx, \;I_1=\int xe^{ax}dx,\; I_2=\int x^2e^{ax}dx$. By induction find $I_n=\int x^ne^{ax}dx$ where $n$ is a positive integer.
# 
...
# 
# ### Q23*
# (a) Integrate $I_1 = \int_0^\infty xe^{-ax^2} dx$ by parts by choosing $v = e^{-ax^2}$ and $u = 1$. This is an alternative method to that in Q17.
# 
# (b) Integrate $I = \int_0^\infty n^n e^{-ax^2} dx$ by parts to find a recurrence relationship between $I_n$ and $I_{n+2}$.
# 
...
# ### Q28
# 
# (a) find $\int_{-\pi}^{\pi}dx$,
# 
# (b) Show that $\displaystyle \int_{-\pi}^\pi e^{imx}e^{-inx}=2\pi\delta_{n,m}$, where $m$ and $n$ are integers. If $m = n$ then the delta function $\delta_{n,m} = 1$, otherwise it is zero.
# 
# ### Q29
...
# 
# Evaluate the Sine Integral $\displaystyle Si(w)=\int_0^w\frac{\sin(x)}{x}dx$ but with an upper limit of infinity, i.e. $\displaystyle I =\int_0^\infty \frac{\sin(x)}{x}dx$, by first multiplying  by $e^{-\beta x}$ to produce $\displaystyle I =\int_0^\infty e^{-\beta x}\frac{\sin(x)}{x}dx$ and then differentiating w.r.t. $\beta$.
# 
# Next integrate twice over, first with respect to $x$ and then with respect to $\beta$. Use the limits zero and infinity for the first integration. Finally calculate what happens when $\beta = 0$, thus making the exponential term unity and so returning the original integral. 
# 
