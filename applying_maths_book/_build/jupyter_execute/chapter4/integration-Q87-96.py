...
# 
# (b) The work $w$ required to expand a gas is the line integral $w = -\int p dV$. If $T$ and $p$ are the variables to be used, this equation can be written as $\displaystyle w =\int p\left(\frac{\partial V}{\partial T}\right)_p dT+p\left(\frac{\partial V}{\partial p}\right)_T dp$. For 1 mole of an ideal gas calculate $w$ along each of the two paths used in the example in Section 13.9 and Figure 36 and hence show that $w$ is not a state function.
# 
