...
# 
# (b) In this part $g(x) = x^2$ then the integrals are $\displaystyle M_x=\frac{1}{2}\int_0^r (r^2-x^2-x^4)dr =\frac{r^3}{2}\left(\frac{2}{3}-\frac{r^2}{5} \right),\; M_y=\int_0^r x\sqrt{r^2-x^2}dx$. Substituting for the square root using a sine gives the result $M_x=-(r^2-x^2)^{3/2}/3$ which gives the same value as for $M_y$, as expected, because of symmetry.
# 
# The average $x$ and $y$ positions are obtained by dividing the centroids by the area which for a quadrant is $\pi r^2/4)$; see equation 35; $\langle y\rangle  = M_x /A, \,\langle x\rangle  = M_y /A$. The average values are $4r/3\pi$.
# 
...
# 
# ### Q50 answer
# (a) The volume of a small circular disc of thickness $dx$ is $\pi r^2dx$ where $r$ is the radius of the disc at any position $x$. The mass of the element is therefore $\rho_0\pi r^2dx$ where $rho_0$ is the density (mass / unit volume) and the total mass of the lens is $\displaystyle \pi\rho_0\int_0^h r^2 dx= a\pi\rho_0\int_0^h x dx = a\pi\rho_0 h^2/2$.
# 
...
# 
# (b) Since now the density is a function only of distance $x$ then it is uniform in the y-direction. The mass is the integral of the volume element $\rho_(x)\pi r^2dx$ over the length, which is $\displaystyle a\rho_0\pi\int_0^h(1+bx)xdx=a\rho_0\pi\left( \frac{h^2}{2}+b\frac{h^3}{3} \right)$.
# 
...
# 
# ### Q51 answer
# (a) The volume is $\displaystyle V=\pi /int_0^\infty y^2dx=\pi\int_0^\infty \frac{dx}{x^2}=\pi$. The surface area is given by $\S=2\pi\int_a^b y\sqrt{1+y'^2}dx$, where $y'$ is the derivative. This gives $\displaystyle S=2\pi\int_1^\infty \frac{1}{x}\sqrt{1-\frac{1}{x^2}}dx=2\pi\int_1^\infty \sqrt{\frac{x^2-1}{x^4}}dx$. The integration is quite tricky and produces infinity, using Sympy is easy.
...
# The surface area returned is infinite. This is really a very strange result; in fact, it seems to be nonsensical. The volume is finite therefore the horn can be filled with something, a volume equal to $\pi$ dm$^3$ (litres) of paint for example. Its inner surface would be covered but its outer surface area needs an infinite amount of paint to cover it. Repeating the calculation to some finite length, say $100$, produces a similar result; the surface area is far greater than the volume, but is this not a misleading comparison?  The units involved are different. The volume is measured in m$^3$, area in m$^2$. Therefore if $\pi$ litres is used to fill the volume of an infinitely long horn, the surface can be still be covered with this amount of paint but only with an infinitely thin layer. As you will have realized this is not possible. Consider the case when the length is for instance $9$ dm, and the integral extends from $1 \to 10$. The area is now $4.83 \pi$ dm$^2$ and the volume $0.9\pi$ dm$^3$, which means that the coat of paint can now be $\approx 0.18$ dm thick.
# 
...
# (b) Using the results just calculated the variance of the energy is $\sigma_E^2 = \langle E^2\rangle - \langle E\rangle^2 = (k_BT)^2$ and the standard deviation compared to the average energy is $\sigma_E /\langle E\rangle = 1$, which, for calculated for a single molecule is a huge fluctuation in energy. This is not observed in practice because of the huge numbers of molecules in a mole. Thus for $N$ molecules $\sigma_E^2 = \langle E^2\rangle - \langle E\rangle^2 = (Nk_BT)^2$ and  then $\sigma_E /\langle E\rangle = \sqrt{N}/N=1/\sqrt{N}$. 
# 
...
# 
# ### Q55 answer 
# (a) The variable to be differentiated with respect to is not the same as that for integration therefore the differentiation can be placed inside the integration as $ \displaystyle \frac{d}{dT}\int f(x,T)dx=\int\frac{d}{dT}f(x,T)dx$.
# 
# (b) Using the chain rule to differentiate $\ln(y)$ produces $\displaystyle \frac{d\ln(y)}{dx}=\frac{1}{y}\frac{dy}{dx}$ or in general for any function $\displaystyle \frac{d\ln(f(y))}{dx}=\frac{1}{f(y)}\frac{dy}{dx}$.
# 
...
# ### Q56 answer
# (a) The average speed is by definition, $\displaystyle \langle u\rangle =\frac{\int_0^\infty uP(u)du}{\int_0^\infty P(u)du} = \int_0^\infty uP(u)du$ where the second step follows because the distribution is normalised. Substituting produces $\displaystyle \langle u\rangle =4\pi\left( \frac{m}{2\pi k_BT} \right)^{3/2}\int_0^\infty u^3e^{-mu^2/2k_BT} du$, which has a form met before but which can easily be recalculated (assuming that a > 0) or looked up in the integration table (Section 2.13). Using Sympy gives
...
# 
# The root mean square speed is calculate using $\displaystyle \langle u^2\rangle = \int_0^\infty u^2P(u)du$, which is an integral of the form $\displaystyle \int u^4e^{-au^2}du =\frac{3\sqrt{\pi}}{8a^{5/2}}$ which makes the root mean square speed $\displaystyle \sqrt{\langle u^2\rangle} =\sqrt{ \frac{3 k_BT}{ m} }$. 
# 
# (b) The most probable speed occurs at the maximum $P(u)$ and is found by differentiating the speed distribution and setting the result to zero, $\displaystyle \frac{dP(u)}{dU}=0= (2k_BT-mu^2)\frac{\sqrt{2}m^{3/2}}{\sqrt{\pi}(k_BT)^{5/2}}ue^{-mu^2/2k_BT}$ from which $\displaystyle u_{max_p}= \sqrt{\frac{2k_BT}{m}}$ which is the most probable, but not, maximum speed.
# 
...
# With a similar calculation as for the average energy, the mean square energy is $\displaystyle \langle E^2 \rangle=\frac{\int_0^\infty E^{5/2}e^{-E/k_BT}dE}{\int_0^\infty E^{1/2}e^{-E/k_BT}dE}=\frac{15}{4}\left(k_BT\right)^2$ and so the dispersion of the energy is $\displaystyle \sigma_E^2 =\langle E^2 \rangle - \langle E \rangle^2 =\frac{3}{2}(k_BT)^2$ and the fractional standard deviation  $\displaystyle \frac{\sigma_E}{\langle E \rangle}=\sqrt{\frac{2}{3} }$.
# 
# ### Q57 answer
# (a) The displacement, not the energy, is calculated. The probability function to use is the Boltzmann distribution with the energy at position $x$. In the harmonic oscillator, it is intuitive that the average displacement is zero; this can also be appreciated from the equation $\displaystyle \langle x \rangle =\frac{\int xe^{-kx^2/(2k_BT)}dx}{\int e^{-kx^2/(2k_BT)}dx}$ where the limits are $\pm \infty$. The numerator is an odd function and, when calculated over a symmetrical range of displacements, is zero, therefore $ \langle x \rangle=0$. 
# 
# The mean square displacement is not zero as both numerator and denominator are even functions $\displaystyle \langle x^2 \rangle =\frac{\int x^2e^{-kx^2/(2k_BT)}dx}{\int e^{-kx^2/(2k_BT)}dx}$. Both integrals are standard forms and can also be calculated with Sympy in a manner shown in the previous answer, $\displaystyle \langle x^2 \rangle= \frac{k_BT}{k}$ and hence $\displaystyle  \langle x^2 \rangle-\langle x \rangle^2= \frac{k_BT}{k}$.
# 
...
# 
# ### Q58 answer
# The motion is $x = x_0\cos(\omega t)$ where $\omega$ is the angular frequency and $x_0$ the maximum amplitude. The small distance $dx$ is proportional to $dt$ and by differentiation $dx = =\omega x_0 \sin(\omega t)dt$. The time $dt$ taken to go a distance $dx$ is therefore $\displaystyle dt=-\frac{dx}{\omega x_0\sin(\omega x)}$ and the time for one period is $T=1/v=2\pi/\omega$. Because $\cos(\omega t) = x/x_0$, using Pythagoras (right angled triangle) produces $\sin(\omega t) = \sqrt{x_0^2-x^2}/x_0$ and $dt=-dx/(\omega\sqrt{x_0^2-x^2})$. The fractional time to move $1/2$ a period from $x_0 \to -x_0$ is
# therefore, $\displaystyle \frac{dt}{T}=\frac{1}{\pi}\frac{dx}{\sqrt{x_0^2-x^2}}$.
# 
# This equation means that as the fractional time equals the fractional distance, the probability distribution is $\displaystyle P(x)= \frac{1}{\pi}\frac{dx}{\sqrt{x_0^2-x^2}}$. This function is approximately 'U' shaped about the equilibrium bond extension position. At the maximum extension $\pm x_0,\, P(x_0)=\infty$ and at $x=0$ the probability is a minimum and has the value $1/(\pi x_0)$.
# 
# This distribution is infinite at $x = \pm x_0$ but surprisingly is normalized as can be shown by integrating between the limits of the motion $\displaystyle I=\int_{-x_0}^{x_0} P(x)dx= \frac{1}{\pi}\int_{-x_0}^{x_0}\frac{dx}{\sqrt{x_0^2-x^2}}$. This is a standard integral $\displaystyle I=\frac{1}{\pi}\sin^{-1}\left(\frac{x}{x_0} \right)\bigg|_{-x_0}^{x_0}=1$  where $\sin^{-1}(-1)=-\pi/2$.
# 
# (c) The average position is zero because the function to integrate is odd, the probability distribution is symmetrical about $x = 0$ and the extension and the limits are symmetrical. As the distribution is normalised  $\langle x\rangle = \int xP(x)dx=0$. The mean squared displacement is $\langle x^2 \rangle = \int x^2P(x)dx$ and is quite an awkward integration. Using Sympy gives $\langle x^2 \rangle = x_0^2/2$ as shown below. The dispersion, or variance is $\langle x^2 \rangle - \langle x \rangle^2$ which is also equal to $x_0^2/2$ because $\langle x \rangle  = 0$.
...
# ### Q59 answer
# (a) The probability distribution function is $\psi^*\psi dx$ making the integral $\langle x \rangle = \int_{-\infty}^\infty x\psi^2(x)dx$ because $\psi$ is real not complex. First consider the wavefunction for the second quantum level $\psi_2$, which is an even function, and therefore $\psi_2^2$ is also even, but multiplying by $x$ makes $x\psi_2^2$ odd. Therefore this and all even quantum numbered wavefunctions produce $\langle x \rangle = 0$.
# 
...
# 
# Calculating $\langle x^2 \rangle=\int_{-\infty}^\infty x^2\psi^2(x)dx$ produces even functions which are not identically zero and have to be evaluated. Using Sympy the first thing to check is that the wavefunctions (Q39) are normalised then $\int\psi^2 dx=1$.
...
# Next calculate $\displaystyle \langle x^2 \rangle=\int_{-\infty}^\infty x^n\psi_n^2(x)dx$ for a range of $n$ values. The general harmonic oscillator wavefunction is $\displaystyle \psi(n,x)=\frac{1}{\sqrt{2^nn!}}\left(\frac{\alpha}{\pi} \right)^{1/4}H_n(\sqrt{\alpha}x)e^{-\alpha x^2/2}$  where $\alpha = \mu\omega/\hbar= \sqrt{k\mu}/\hbar$ with reduces mass $\mu$ and force constant $k$.
...
# ____
# 
# (e) The average momentum is $\displaystyle  \langle p\rangle =-i\hbar\int_{-\infty}^\infty \psi^*(x)\frac{d}{dx}\psi(x)dx$. Because the motion is equally to the left and right during the vibration the average momentum should be zero. 
# 
# To check this start with the $n = 0$ wavefunction $\psi_0(x)=N_0e^{−\alpha x^2/2}$, where $N_0 = (\alpha/\pi)^{1/4}$ is the normalization constant. The average momentum is therefore 
# 
...
# 
# Using Sympy the result is $\displaystyle \langle p^2 \rangle = \alpha\hbar^2 N_0^2\frac{1}{2}\sqrt{\frac{\pi}{\alpha} }=\alpha\frac{\hbar^2}{2}$ after simplifying and substituting for the constants.  
...
# 
# With states other than $n = 0$ the uncertainty principle also applies. The harmonic oscillator is somewhat unusual in that the lowest eigenstate exactly matches the uncertainty principle. In many quantum systems, such as particle in a box, each eigenstate has an average position - momentum $$ \sqrt{\langle x^2 \rangle \langle p^2 \rangle} \ge \frac{\hbar}{2}$$
# 
# ### Q60 answer
...
# 
# The average oscillator energy is by definition $\displaystyle \langle E \rangle= \int_0^\infty EP(E)dE\,\big/ \int_0^\infty P(E)dE$ and if the distribution is normalised then the denominator is unity. 
# 
# It is assumed for the purpose of the question that the averages can be obtained by integration rather than summation. The normalisation is $\displaystyle \int_0^\infty \frac{\beta^s E^{s-1}}{(s-1)!}e^{-\beta s}dE$
...
# which when simplified has the value unity, because the gamma function $\Gamma(s)=(s-1)!$. The average is therefore $\displaystyle \langle E \rangle=\int_0^\infty \frac{\beta^s E^s}{(s-1)!}e^{-\beta s}dE$
...
# where again $\Gamma(s)=(s-1)!$ is used to simplify. The average squared $\displaystyle \langle E^2 \rangle=\int_0^\infty \frac{\beta^s E^{s+1}}{(s-1)!}e^{-\beta s}dE= \frac{s(s+1)}{\beta^2}$. The variance is therefore $\displaystyle \sigma^2=\langle E^2 \rangle -\langle E \rangle^2  = \frac{s}{\beta^2}$ and the standard deviation $\sigma =k_BT\sqrt{s}$ where $\beta=1/k_BT$.
# 
...
# 
# ### Q61 answer
# (a) The normalization is obtained by calculating the probability over all space which has to be $\displaystyle \int_0^L \psi_n^*(x)\psi_n(x)dx=1$ and is therefore $\displaystyle N\int_0^L \sin(n\pi x/L)^2 dx=1$. This integral is $L/2$ as $n$ is an integer making $N=\sqrt{L/2}$. The normalised wavefunction is $\displaystyle \sqrt{\frac{L}{2} }\sin(n\pi x/L)$
# 
# (b) The average $\displaystyle \langle x \rangle =\int_0^L \psi_n^*(x)\,x\,\psi_n(x)dx$ and should have the answer $L/2$ as the box is symmetrical about its centre. As a check the integral is 
...
# The average of $x^2$ depends on the quantum number and is $\displaystyle L^2\frac{(2n^2\pi^2-3)}{(n\pi)^2}$ which produces a variance $\displaystyle \sigma_x^2=\langle x^2 \rangle - \langle x\rangle ^2= L^2\left(\frac{1}{12}-\frac{1}{2n^2\pi^2}  \right)$ and this function is a measure of the spread of the wavefunction. When $n = 1$ the wavefunction is half a sine wave without a node and most of the amplitude is close to the centre of the box. As $n$ increases, more nodes are present and the wavefunction spreads out more evenly over the box. As the probability is more evenly spread, the variance $\sigma_x^2$ becomes larger showing that the wavefunction is less localized.
# 
...
# 
# ### Q62 answers
# Collecting together the terms the equation to use for energy level $k$ is, $\displaystyle \langle E_k\rangle =-\frac{\hbar^2}{2m}\int_0^L\psi_k^*(x)\frac{d^2}{dx^2}\psi_k(x)dx$. Differentiating the wavefunction twice gives $\displaystyle -\left(\frac{k\pi}{L}\right)^2\sqrt{\frac{2}{L} }\sin(k\pi x/L)$ making the integral $\displaystyle \langle E_k\rangle =-\frac{\hbar^2}{m}\left(\frac{k\pi}{L}\right)^2\int_0^L\sin\left(k\pi \frac{x}{L}\right)^2dx$. 
# 
# This integral can easily be evaluated by converting the sine to its exponential form. The result is $\displaystyle \langle E_k\rangle =\frac{\hbar^2}{2m}\left( \frac{k\pi}{L}\right)^2$. Notice that the bigger the box the smaller the energy because the electron is more spread out. Also the larger the mass, the smaller the energy.
# 
# The calculation in Sympy can be done easily by forcing $k$ to be an integer.
...
# ### Q63 answer
# The transition probability can be found from equation 38 and the wavefunctions for the particle in a box, $\displaystyle \psi_n(x)=\sqrt{2/L}\sin(2\pi x/L)$. The initial state has level $n$ the final $k$. The average is
# 
# $$\displaystyle \langle x_{n,k}\rangle  = q\int_0^L \psi_n^*(x) \left(x-\frac{L}{2}\right )\psi_(x)dx
# = q\int_0^L \psi_n^*(x)\,x \,\psi_k(x) dx - q\int_0^L \psi_n^*(x) \frac{L}{2}\psi_k(x)dx $$
# 
# and the second integration is identically zero because of the orthogonality of the wavefunctions with different quantum numbers $n$ and $k$. The first integral is $\displaystyle \langle x_{n,k}\rangle  = q\frac{2}{L}\int_0^L \sin(n\pi,x/L)\,x\,\sin(k\pi x/L)dx$. 
# 
# To solve this integral convert the sine terms into sums and differences using the trig formula $2 \sin(a)\sin(b) = \cos(a - b) - \cos(a + b)$ then integrate by parts the two terms produced. Alternatively, the functions can be converted to their exponential form. Using Sympy gives the result directly which is $\displaystyle \langle x_{n,k}\rangle = \frac{4L}{\pi^2}\frac{nk((-1)^{k+n}-1)}{(n-k)^2(n+k)^2}$. 
...
# 
# The integrated transition intensity is $A_{n,k} = \beta \nu_{n,k} |\langle \mu_{n,k} \rangle |^2$ where $\displaystyle \beta = \frac{8\pi^2N}{3000hc}$. Substituting for the frequency, using the energy gap calculated using equations in Q62, the transition frequency is $\displaystyle \nu_{n,k}= \frac{\hbar}{4\pi m}\left( \frac{\pi}{L} \right)^2(k^2-n^2)$. As the intensity is proportional to $\nu_{n,k} |\langle \mu_{n,k} \rangle |^2$ and substituting for these terms shows that the intensity is independent of the length of the  box $L$.
# 
# 
# The lowest energy transition is $\displaystyle \nu_{4,5}=9 \frac{\hbar}{4\pi m}\left( \frac{\pi}{L} \right)^2$ with intensity $A_{4,5}\approx 0.36\beta$. The intensity of the transition $4\to 7$ is $\approx 0.0143\beta$ which is considerably weaker than for the lower energy transition. The reason for this is that the higher wavefunctions have more nodes, but with a smaller period and therefore there is more canceling between the two wavefunctions.
# 
# 
...
FC00 = abs( integrate(psi(0,x_a)*psi(0,x_b),(x,-oo,oo)) )**2  #  FC 0-0
simplify(FC00)
...
FC01=abs( integrate(psi(0,x_a)*psi(1,x_b),(x,-oo,oo)) )**2  # FC 0-1
factor(simplify(FC01) )


# In[47]:


FC05 = abs( integrate(psi(0,x_a)*psi(5,x_b),(x,-oo,oo)) )**2  # FC 0-5
factor(simplify(FC05) )


# In[48]:


FC06 = abs( integrate(psi(0,x_a)*psi(6,x_b),(x,-oo,oo)) )**2  # FC 0-6
factor(simplify(FC06) )


# Looking at these integrals, notice that the exponential terms are all the same and contain
# $X = \alpha(x_a - x_b)^2$ giving $FC_{0\to 0} = e^{-X/2}/2$. Looking at the other transitions the constants can be explained as $\alpha^n/(2^n n!)$ making the general expression $\displaystyle FC_{0 \to m} = \frac{\alpha^m}{2^m m!}e^{-X/2}$.
# 
# c) Plotting the FC factors vs quantum number with reduced displacement $X = 1$ and $5$, produces the graphs below. When there is a small displacement, most of the intensity of the transition, as controlled by the FC factors, goes into vibrational levels with small quantum numbers of state $b$, whereas, when the displacement is large, higher state $b$ quantum numbers are excited Figure 56.
# 
...
# 
# When the concentration is $c$ then $\displaystyle \phi=\int_0^\infty e^{-k_ft-(c/c_0)\sqrt{\pi k_ft}} dt\big/\int_0^\infty e^{-k_ft} dt$.  The denominator evaluated to $1/k_f=\phi_0$, see equation 29. Using Sympy for the more complicated integral with $a=c/c_0$ gives
# 
...
# which produces the ratio $\displaystyle \frac{\phi}{\phi_0}=1 -\frac{\pi c}{2c_0}\text{erfc}\left( \frac{\sqrt{\pi}c}{2c_0} \right)e^{\pi (c^2/2c_0)^2}$  and where the complimentary error function $\text{erfc(x)}=1-\text{erf}(x)$. If $x\equiv \sqrt{\pi}c/(2c_0)$ (which is dimensionless) then the equation is simplified to 
# 
...
# ### Q66 answer
# To use l'Hopital's rule the expression must be put as a fraction, thus $\displaystyle \lim_{x\to\infty}=\frac{x\sqrt{\pi}(1-\text{erf}(x))}{e^{-x^2}}$. The next step is to differentiate top and bottom separately. The error function is defined as $\displaystyle \text{erf}(x)=\frac{2}{\sqrt{\pi}}\int_0^x e^{-u^2}du$ has the derivative $\displaystyle \frac{2}{\sqrt{\pi}} e^{-x^2}$. Repeatedly differentiating to and bottom of the fraction separately gives
# 
...
# The result is $\langle \cos^2(\theta)\rangle =1/3$ which proves that $\displaystyle \langle \frac{3\cos^2(\theta)-1}{r^3} \rangle =0$ and shows that direct dipole-dipole coupling cannot be measured in solution by using NMR. The spin-spin coupling measured in solution is the indirect dipole-dipole coupling (J coupling) of the nuclear spins mediated via the electrons.
# 
# ### Q68  answer
# (a) The first calculation is to show that the total
# wavefunction is normalized then by using the definition of
# an average equation 28 calculate the answers to (b).
# 
# Integrating the angular terms in the normalization equation 46 produces $\displaystyle \int_0^\pi \sin(\theta)d\theta=2$ and as $\displaystyle \int_0^{2\pi}d\varphi =2\pi$ then substituting for the 1s wavefunction gives,
# 
...
# has the form, $\displaystyle \int_0^\infty x^2e^{-ax}dx=\frac{2}{a^3}$. Substituting for the values $\displaystyle I=4\left(\frac{Z}{a_0}  \right)^3\frac{2a_0^3}{8Z^3}=1$ which confirms that the wavefunction is normalised.
# 
...
# 
# The general result is $\displaystyle \langle r^n \rangle=2^{-1-n}\left(\frac{a_0}{Z}\right)^n (n+2)!$
...
# 
# The integration is done by parts and is simple but involved, instead using Sympy with an upper limit of $a_0$ ,gives $p_{0\to a_0}= (e^{2Z}-2Z-2Z^2-1)e^{-2Z}$.
...
# 
# The integrals can be solved with the substitution $a=\epsilon\mu /k_BT$ and $u=\cos(\theta),\, du=-\sin(\theta)d\theta$. The denominator becomes $\displaystyle \int_0^1\sin(\theta)e^{a\cos(\theta)}d\theta=\int_{-1}^1 e^{au}du= \frac{e^a-e^{-a}}{a}$. The denominator can be treated similarly and produces $\displaystyle \int_{-1}^1 ue^{au}du=(e^{-a}+ae^{-a}-e^a+ae^{a})/a^2$.
# 
# The average energy is the ratio of these last two results and is $\displaystyle \langle E \rangle =-\epsilon\mu \left(\frac{e^a+e^{-a}}{e^a-e^{-a}}-\frac{1}{a}\right) =-\epsilon\mu \left(\coth(a)-\frac{1}{a}\right) $. Physicists call the term in brackets the Langevin function and substituting for constants gives  $\displaystyle \langle E \rangle =-\epsilon\mu\coth\left(\frac{\epsilon\mu}{k_BT}\right)-k_BT $. 
# 
...
# As $a \ll 1$ then $a^3$ and higher powers are insignificant therefore the result is for the average energy is $\displaystyle \langle E\rangle= -\frac{a\epsilon\mu}{3}=-\frac{\epsilon^2\mu^2}{3k_BT} $ provided that $\epsilon\mu \ll k_BT$. 
# 
# As a check work out the units. The electric field has units
...
# 
# The mean square energy $\langle E^2 \rangle$ is calculated in a similar way using energy $E^2$ in the calculations instead of $E$. Using the substitution $a=\epsilon\mu/k_BT$, as above, $\displaystyle \int_{-1}^1 u^2e^{au}du= \frac{e^{-a}}{a^3}\left( -(a^2+2a+2)+(a^2-2a+2)e^{2a} \right) $. Then dividing by the normalisation gives 
# 
# $$\displaystyle \frac{\langle E^2 \rangle}{\epsilon^2\mu^2}=\frac{ -(a^2+2a+2)+(a^2-2a+2)e^{2a}  }{a^2(e^{2a}-1)}$$
# 
# Expanding the exponentials in the limit $a\ll 1$ gives $\displaystyle \langle E^2 \rangle=\frac{\epsilon^2\mu^2}{3}$ which means that this can be re-written in terms of the heat capacity as $\displaystyle \langle E^2 \rangle=k_BT^2C_V$.
# 
# The dispersion or variance at small field values is $\displaystyle \sigma^2=\langle E^2 \rangle-\langle E \rangle^2=\frac{(\epsilon\mu)^2}{3}-\frac{(\epsilon\mu)^4}{(3k_BT)^2}= \frac{(\epsilon\mu)^2}{3}\left(1-\frac{(\epsilon\mu)^2}{3k_BT}\right)$ and as $\epsilon\mu/k_BT \ll 1$ then $\sigma^2 \approx (\epsilon\mu)^2/3$.
# 
...
# function is zero because $\mathrm{erfc(small)} \to 1$. In addition, the exponential term can be expanded for the same reason and produces $\displaystyle \langle J\rangle=\sqrt{\frac{\pi k_BT}{4B}}\left( 1+\frac{B}{4k_BT} \right)-\frac{1}{2}$.
# 
...
# 
# ### Q71 answer
# (a) Substituting for $g$ produces $\displaystyle Z=\frac{4\pi\sqrt{2}Vm^{3/2}}{\hbar^3}\int_0^\infty \sqrt{\epsilon}\,e^{-\epsilon/k_BT}d\epsilon$.  Integration of this form involve the error function and can be started with the substitution $u=\sqrt{\epsilon}$ and using integration by parts. The error function evaluates to zero and 1 with the limits zero and infinity respectively making the integral; $\int_0^\infty \sqrt{\epsilon}\,e^{-\epsilon/k_BT}d\epsilon=\sqrt{\pi}(k_BT)^{3/2}/2$. The translational partition function $\displaystyle Z=V\left(\frac{2\pi mk_BT}{\hbar^2}\right)^{3/2}$. The integration via Sympy is shown below.
...
# ### Q72 answer
# (a) The expectation has the usual form $\langle p_x \rangle=\int \psi^*p_x\psi dx\big/\int \psi^*\psi dx$. The normalisation (denominator) is a constant equal to $\int dx$ but the numerator is 
# 
...
# 
# (b) the expectation $\displaystyle \langle p_x^2 \rangle=(-i\hbar)^2\int e^{-ip_x x/\hbar}\frac{d^2}{dx^2} e^{ip_x x/\hbar} dx\big/ \int dx = p_x^2$.
# 
