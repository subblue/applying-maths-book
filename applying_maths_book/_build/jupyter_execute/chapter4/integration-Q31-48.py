...
# (a) $r = a \cos(\theta)\sin(\theta)$, 
# 
# (b) $r = a \sin(3\theta + \pi/3)$, 
# 
# (c) $r = n \cos(\theta) + \cos(n\theta)$ when $n$ is an odd integer.
# 
...
# 
# ### Q33
# Figure 13 shows the hyperbolic curve<sup>#</sup> $x^2 - y^2 = 1$. If OA = $\cosh(\theta)$, prove that AB = $\sinh(\theta)$ and show that the area QAB=$\displaystyle \int_0^\theta \sinh^2(\theta) d\theta$. Find the area of the triangle OAB and of OQB.
# 
...
# 
# In quantum mechanics, the Hamiltonian operator is $\displaystyle H= -\frac{\hbar^2}{2m}\frac{\partial^2}{\partial x^2}+V(x)$ which is the differential operator $\displaystyle -\frac{\hbar^2}{2m}\frac{\partial^2}{\partial x^2}$ that describes the kinetic energy, plus $V(x)$ the potential energy. The differential equation is then $H\psi=E\psi$ where $H$ operates on the wavefunction $\psi$
# 
...
# 
# ### Q37
# Find the area under the curve $\displaystyle x^2e^{-\alpha x^2}$ from $x = x \to \infty$ where $x \ge 0$ and $\alpha \gt 0$.
# 
# ### Q38
# Use a computer algebra application such as Sympy to investigate integrals of the form $\int x^ne^{-\alpha x^2}dx$.
# 
# ### Q39*
...
# \end{array}$$
# 
# where $N_0 = (\alpha/\pi)^{1/4}$ and $\alpha = \sqrt{k\mu}/\hbar$, $k$ being the force constant and $\mu$ reduced mass. By inspection, classify the wavefunctions into odd or even functions.
# 
# ### Q40
...
# 
# ### Q41
# In an SIS type of disease an individual is either susceptible $S$ or infected $I$, for example the common cold and influenza. Such infections do not give immunization upon recovery from infection, i.e. there is no long lasting immunity and individuals become susceptible again. The scheme is
# 
...
# 
# If the number infected is exponentially distributed $I(t) = e^{-k_2t}$ and if $n(t) = n_0e^{rt}$, find $r$, the initial per capita growth rate of the disease. (Convolution is described in Chapter 9.)
# 
# ### Q42
...
# 
# (b) Suppose that you could remove an ion from the water, $ε_{H2O} = 80$, and place it into a lipid bilayer membrane of $\epsilon_m = 2$. What would be the energy required to do this if the ion is K+ which has a radius of $0.22$ nm?
# 
