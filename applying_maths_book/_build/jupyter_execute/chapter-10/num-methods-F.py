...
#Algorithm 17  modified Euler procedure
...
def mod_euler(dydx,dzdx,xvals, x0,y0,z0,xn,n):  # Modified Euler or RK2 method
    ...
    Eulerz[0] = z0
    xvals[0]  = x0
    for i in range(1,n):      # modifiued Euler 
        k1= dydx(y,z)
        L1= dzdx(y,z)
        k2= dydx(y,z+L1*h)
...
Q = 1e-6       # choose limit on accuracy of bisection
...
while k < 5 and np.abs(yn0-yxn) > Q:                        # start iterative loop to find solution
    yvals,zvals = mod_euler( dydx,dzdx,xvals, x0,y0,alpha1,xn,n)
    yn1   = yvals[n-1]
...
# Algorithm 20 Shooting Method used to solve the Schroedinger equation.
...
for indx in [0,1]:                                    # do odd-even values
    ...
    print('{:s}'.format('quantum num, eigenvalue'))
    while En1 < maxE :                                # keep on to max energy
        ...
        if yN1*yN0 < 0.0 :                            # check if zero crossed
            ...
            print('{:5d}   {:12.6f} '.format( Qn, En1)) # results
                                                        # plot energy levels and wavefunctions here
...
# 
# The potential energy (Swalen & Ibers 1962), $V$, of the umbrella or tunnelling normal mode vibration, measured as the displacement $x$ of the N atom from the plane of the H atoms, assuming a $C_{3V}$ point group, is approximated by $\displaystyle V(x) = 30471x^2 + 12478e^{−5.005x^2} - 10458$ cm<sup>-1</sup>. The displacement $x$ is in $\overset{\text{o}} A$. The eigenvalues and their spacing will be calculated up to an energy of 4000 cm<sup>-1</sup>, which is above the barrier.
# 
# The normal SI units for Planck's constant and the reduced mass are difficult to use because they generate very large or very small numbers. Converting to atomic units greatly simplifies the calculation. This is done by using an energy scale in hartree, where 1 hartree is $2.1947465225 \cdot 10^5$ cm<sup>-1</sup> or 27.211396 eV, and the corresponding distance scale, is in units of the Bohr radius $a_0$, which is $0.529177 \cdot10^{-10}$ m. The hartree is the natural unit of energy and is $e^2/(4\pi\epsilon_0a_0)$, where $\epsilon_0$ the permittivity of free space and $e$ the charge on the electron. The constants in the Schroedinger equation now become $\hbar^2/m_e = 1$, where $m_e$ is the mass in kg of the *electron* and therefore for the reduced mass $\mu$ (kg) of the molecular vibration, the ratio $\displaystyle \frac{\hbar^2}{m_e}\frac{m_e}{\mu}\equiv \frac{m_e}{\mu}$ is used. The schroedinger equation is now 
# 
# $$\frac{d^2\psi}{dx^2}-\frac{2\mu}{m_e}\left(V(x)-E\right)\psi=0$$
# 
...
# 
# Plotting this potential shows that the calculation needs an $x$ range of 0 to $2a_0$ and an energy scale up to 0.02 hartree, Figure 30. The potential is essentially parabolic, except for the low and Gaussian shaped barrier of $\approx$ 0.0094 hartree (2063 cm<sup>-1</sup>), centred at zero, which produces two energy minima at $\pm 0.723 a_0$.
# 
# In the numerical calculation, energy increments of no more than 0.001 are needed to determine all the levels accurately. The reduced mass is assumed to be that of three hydrogen atoms acting as one mass, and the nitrogen atom. The masses in kg are $m_H = 1.673534 \cdot 10^{-27},\;  m_N = 2.325267 \cdot 10^{-26}, \; m_e = 9.1093819 \cdot 10^{-31}$, making $\mu/m_e = 4532.769$. 
# 
...
# 
# The energy gaps, in cm$^{-1}$, compared with the spectroscopically determined values are shown below.
# 
# $$\displaystyle \begin{array}{ccc}
#  &\text{Table 3}\\
