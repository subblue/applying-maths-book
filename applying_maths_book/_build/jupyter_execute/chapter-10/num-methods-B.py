...
# 
# $$\displaystyle \frac{dy}{dt}=f(t,y)  \tag{16}$$
# 
...
# 
# $$\displaystyle  \frac{\Delta y}{\Delta t}\approx \frac{dy}{dt}=f(t,y)  \tag{17}$$
# 
...
# 
# $$\displaystyle  \frac{\Delta y}{\Delta t}= \frac{y_{n+1}-y_n}{h}\approx f(t_n,y_n)  \tag{18}$$
# 
# Rearranging gives the Euler method equations 
# 
# $$\displaystyle  y_{n+1}=y_n+hf(t_n,y_n)  \qquad \qquad (a) \tag{19}$$
# 
# $$\displaystyle  y_0y(t_0), \qquad t_n=t_0+nh, \qquad n=0,\,1,\,2\cdots N-1 \qquad (b)$$
# 
...
# 
# $$\displaystyle y_{n+1} = y_n - hf(t_n, y_n) \tag{20}$$
# 
# ### 4. 2 Errors
# 
...
soln0,time0 = Eulerf(dydt,t0,y0,maxt,N)  # return results
...
# 
# <img src='num-methods-fig8.png' alt='Drawing' style='width:400px;'/>
...
# 
# The Euler method is rather crude but is easy to implement and is easily improved. Consider again equation 19. The updated $y$ value was calculated using $y_{n+1} = y_n + hf(t_n,y_n)$, but a better approximation to the area under the curve between $y_{n+1}$ and $y_n$ can be made than $hf(t_n, y_n)$ which is a rectangle, Fig.7. This better approximation makes the area into a trapezoid and this is shown as shaded in the right-hand panel of Fig. 7. Its value is $(h/2)[f(t_n,y_n)+f(t_{n+1},y_{n+1})]$. The better method thus has an update term
# 
# $$\displaystyle y_{n+1}=y_n+\frac{h}{2}\left[f(t_n,y_n)+f(t_{n+1},y_{n+1})\right]  \tag{21}$$
# 
# Some simplifications can now be made by replacing $t_{n+1} = t_n + h$ and $y{n+1} = y_n + hf(t_n, y_n)$, which is equation 19, into the second occurrence of the function to become the Modified Euler formula, which is sometimes also called Heun’s method. The result is
# 
# $$\displaystyle y_{n+1}=y_n+\frac{h}{2}\left[f(t_n,y_n)+f(t_n+h,y_n+hf(t_n,y_n))\right]  \tag{22}$$
# 
# which contains a function within a function. This is more clearly written in three steps by defining new terms $k_1,k_2$;
# 
# $$\displaystyle k_1 =f(t_n,y_n), \quad k_2 =f(t_n +h,y_n +hk_1),\quad  y_{n+1} =y_n +h(k_1 +k_2)/2 \tag{23}$$
# 
...
# 
# $$\displaystyle  r(t_0+\Delta t)+r(t_0-\Delta t)=2r(t_0)+(\Delta t)^2\left( \frac{d^2r}{dt^2} \right)_{t_0}+\cdots \tag{24}$$
# 
# $$\displaystyle  r(t_0+\Delta t)-r(t_0-\Delta t)=2\Delta t r(t_0)+2\frac{(\Delta t)^3}{3!}\left( \frac{d^3r}{dt^3} \right)_{t_0} +\cdots \tag{25}$$
# 
# If the time increment $\Delta t$ is small, the third and higher derivatives can be ignored giving from equation 24
# 
# $$\displaystyle  r(t_0+\Delta t)\approx 2r(t_0)-r(t_0-\Delta t)+\frac{f(t_0)}{m}(\Delta t)^2  \tag{26}$$
# 
...
# 
# $$\displaystyle v(t_0)\approx \frac{r(t_0+\Delta t)-r(t_0-\Delta t)}{2\Delta t}  \tag{27}$$
# 
...
# 
# (1) Define potential energy or force & number of time steps.<br>
# (2) Define initial position $r(t_0)$,velocity t,and force.<br> 
# >Calculate second position at $t_0 - \Delta t$.<br>
# 
# (3) Loop over time steps<br>
# >(i) Calculate new positions and velocities if needed.<br>
# >(ii) Calculate and save quantities to be measured; energy, distance,etc.<br> 
# >(iii) Increment time.<br>
# >(iv) Recalculate forces.<br>
# end loop.<br>
# 
...
# 
# $$\displaystyle  r(t_0+\Delta t)= r(t_0)+v(t_0)\Delta t+\frac{f(t_0)}{m}(\Delta t)^2  \tag{28}$$
# 
...
# 
# $$\displaystyle v(t_0 + \Delta t/2) = v(t_0) + f (t_0)/2m \tag{29}$$
# 
...
# ALGORITHM 12 verlet falling under gravity
...
# 
# $$\displaystyle \left (x_0-\frac{x_0\Delta t}{r}\sqrt{2}\sqrt{E_0-E_0\left(\frac{y_0}{2}\right)^2-U(r)}, y_0  \right) \tag{31}$$
# 
...
# <img src='num-methods-fig10.png' alt='Drawing' style='width:500px;'/>
# 
...
# 
# $$\displaystyle \frac{dC}{dt}=D\frac{d^2C}{dx^2}+ \text{rate expression} \tag{31a}$$
# 
...
# 
# $$\displaystyle C_i^{new} = C_i^{old} + \frac{D\Delta t}{(\Delta x)^2}\left(  C_{i+1}^{old}-2C_i^{old} +C_{i-1}^{old}  \right) \tag{31b} $$
# 
...
# 
# <img src='num-methods-fig10a.png' alt='Drawing' style='width:400px;'/>
# 
# Figure 10 a. The Stencil used to calculate the updating points.
# ______
# 
...
# 
# If a reaction such as $d[C]/dt =-k[C]$ were to be included then the updating equation has the rate term added as
# 
...
# 
# The best method to perform 1D diffusion type calculations, including reaction-diffusion, is,  according to Prest (Numerical Recipes), the Crank-Nicholson approach as it is second order in time and space. This method is still subject to numerical instability as are all numerical methods. The method is computationally more complex than the approach above. The method uses the stencil shown bwloq and is based on using the trapezoidal rule to perform the integration.
# 
# <img src= 'num-methods-fig10c.png'  alt='Drawing' style='width:400px;' />
#      
# Figure 10c. Stencil for the Crank-Nicholson calculation. The top three values labelled with $j+1$ are unknown.
# 
# ______
# 
# The differential equation is expanded in a manner similar to that of eqn 32b but as better approximation is made to the second derivative. The result is 
# 
...
# 
# To solve the equation it is far easier to convert it into matrix form. This means calculating all the spatial dimension terms before calculating in time. For clarity let $\displaystyle \sigma =\frac{D\Delta t }{2(\Delta x)^2} $ and its not so obvious, but it becomes clear later on, that it helps to rewrite the equations by collecting terms in $C^{j+1}$ and in $C^j$ as
# 
# $$\displaystyle -\sigma C_{i-1}^{j+1}+(1+2\sigma)C_i^{j+1}-\sigma C_{i+1}^{j+1} = \sigma C_{i-1}^{j}+(1-2\sigma)C_i^{j}+\sigma C_{i+1}^{j} + f(C_i^j)\Delta t  \tag{31c}$$
# 
...
# 
# this leads to the conditions when $i=0$ that $ C_0^j=C_{-1}^j, \; C_0^{j+1}=C_{-1}^{j+1}$ and when $i=m-1$ then $C_{m-1}^j=C_m^j, \; C_{m-1}^{j+1}=C_m^{j+1}$. The Neumann boundary condition of constant spatial gradient at the limits (in this example the gradient is zero)  prohibits material from entering or leaving across the boundaries; so it is a 'no flux' condition.
# 
...
# 
# $$\displaystyle  S+E \underset{k_-1}{\stackrel{k_1} \leftrightharpoons} ES\stackrel {k_2}\longrightarrow E+P$$ 
# 
# 
# $$\displaystyle \begin{align}
# \frac{d[E]}{dt} =& -k_1[S][E] + (k_{-1}+k_2)[ES] \\[10pt]
# \frac{d[S]}{dt} = & -k_1[S][E] + k_{-1}[ES]\\[10pt]
# \frac{d[ES]}{dt} =& -(k_{-1} + k_2)[ES] + k_1[S][E] \\[10pt]
# \frac{d[P]}{dt} =&  k_2[ES] 
# \end{align}$$
# 
...
# 
# <img src= 'num-methods-fig10d.png'  alt='Drawing' style='width:400px;' />
#      
# Figure 10d. Schematic for microchannel reaction. The reactants are in separate microchannels on the left which opens into a larger single channel. As they flow the fluids react and diffuse together.The entrance width of a channel is typically $50$ microns. The enzyme is flowing down the centre of the channel. The measurement is made at different points across the channels corresponding to different times.
# 
# The calculation is shown below. The initial concentrations are in the first two plot in the top line and the effect at later times below these. The intermediate ES is shown in the third plot and the concentrations vs time measured just in the narrow channel is shown below this.
...
# reaction diffusion  Crank_Nicolson + chemistry    see Schmidt for chemical scheme
...
def do_calc():
    ...
    def Ezvec(Ez,S,ES):    # vector for chemistry dEz/dt  
        for i in range(J ):
            avec[i] = -k1*Ez[i]*S[i] + (km1+k2)*ES[i]      # rate constants dEzined in outside code.
        return avec*dt    
    #########
    
    def Svec(Ez,S,ES):     # chemistry dS/dt  
        for i in range(J ):
            avec[i] = -k1*Ez[i]*S[i] + km1*ES[i]      # rate constants dEzined in outside code.
        return avec*dt    
    #########
    ...
    #print('{:s} {:8.3g} {:8.3g} {:8.3g} '.format('sigma Ez,S,ES,stability <1 ',4*SEz,4*SS,4*SE) )
    ...
    pretty_plot(ax2,xgrid/ms,ES,Ez0,'ES*10 vs time','green')
    ...
    for i in range(1,N):                 # main calculation: iteration in time
        ...
        Eznew= invAEz.dot( BEz.dot(Ez) + Ezvec( Ez,S, ES) )
        Snew=  invAS.dot(   BS.dot(S)  +  Svec( Ez,S, ES) )
        ESnew= invAES.dot( BES.dot(ES) + ESvec( Ez,S, ES) )  
...
dx = dy = 0.002  # intervals in x-, y- directions, mm
...
for m in range(nsteps+1):
    ...
    if m in mfig:                     # plot only at these times
        ...
        im = ax.imshow(u, cmap = plt.get_cmap('viridis_r'), vmin = Tcool, vmax = Thot,interpolation='nearest')
        ax.set_axis_off()
        ax.set_aspect(1)
        if fignum == 1:
        ...
        ax.plot([0.1*nx, 0.1*nx + (0.05/w)*nx],[0.1*ny, 0.1*ny], color='red')     # plot bar size 50 micron
        ax.text(0.1*nx, 0.07*ny, str( r'$50\,\mu m$') )
        ax.plot(xring*nx, yring*nx, color='red')
        av_r = np.sqrt(4*D*1e-6*m*dt)  # average distance diffused at this time ( metres)
        ax.text(0.55*nx,0.1*ny,r'$\langle r\rangle =$'+str('{:6.3g}'.format(av_r*1e6))+r' $\mu m$')
        pass
