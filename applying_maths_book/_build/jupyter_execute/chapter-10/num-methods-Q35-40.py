...
# **Q35** Solve $d^2y/dx^2 = 1/(1 + y^2) + x^2$ with boundary values $y_0 = 10,\; y_5 = -2$. This equation is sensitive to the initial value of $\alpha$.
# 
...
# 
# 
# **Q36** The potential $V(x) = -V_0/\cosh^2(\alpha x)$, sometimes called the Poschl-Teller potential, describes a potential hole and is used to approximate that produced by the diffusion of material in a quantum well or similar device that is used in opto-electronics. The potential is zero at large $\pm x$ and $-V_0$ at the origin, and the constant $\alpha$ determines the width. Calculate all its eigenvalues and wavefunctions if $V_0 = 10$ and $\alpha = 1$. Compare your answer with the analytic eigenvalues $\displaystyle E_n=-\frac{\alpha^2}{8}\left( \sqrt{1+\frac{8}{\alpha^{2}}V_0}-2n-1 \right)^2$
# 
# **Strategy:** Plot out the potential first and estimate the maximum x needed for the integration and also the energy range and increment. Use Algorithm 20 to solve the equations. The energy starts at $-10$ and ends at zero. A step size of $0.1$ is a good starting value. It can be made smaller if you suspect that energies have been missed. Using the shooting method algorithm, as we have defined it, will only work if the minimum energy is zero. This means that $V_0$ should be added to the potential energy and of course subtracted from the eigenvalues at the end of the calculation.
# 
# **Q37** Calculate the eigenvalues and wavefunctions for levels $3, 6, 7$, and $8$ for the potential hill
# $V(x) = +V_0/\cosh^2(\alpha x)$ centred in an infinitely deep square well of width $10$ and with $V_0 = 5$ and $\alpha = 1$ up to $10$ units of energy. Comment on the separation of the eigenvalues.
# 
# **Strategy:** Setting the maximum and minimum $x$ value effectively defines the size of the infinitely deep square well.
# 
# **Q38** The reduced mass used in the ammonia example assumes that the H atoms do not change their relative positions. However, as the N atom tunnels from one side of the plane of the H atoms to the other, the NH bonds must compress. If the transition only involves bending, then the bond lengths must remain unchanged and the reduced mass is now $\mu = 3m_H\left(m_N + 3m_H \sin^2(22^\text{o})\right)/(m_N + 3m_H)$.
# 
...
# 
# **Q39** Calculate the first ten eigenvalues for HCl and I$_2$ using the experimentally determined values for the force constants. How closely do your calculated results come to the experimentally determined eigenvalues supposing that your potential is harmonic?
# 
...
# 
# **Q40** Calculate the first four energy levels for the ns orbitals of the hydrogen atom and compare these with the theoretical values.
# 
