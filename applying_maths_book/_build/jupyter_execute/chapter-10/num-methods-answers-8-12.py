...
#### sample calculation  erome hash to see plot
...
solnp = odeint(dY_dt, C0, tp  )     # solve equations                 
...
# <img src='num-methods-fig31.png' alt='Drawing' alt='Drawing' style='width:350px;'/>
...
# ____
# 
# **Q 10 answer** Using the result of question 7, $\displaystyle \chi=\pi-2\tan^{-1}\left( \frac{1}{\sqrt{d^2/b^2-1}}\right)$ differentiation wrt. $b$ can be performed directly, using the standard result $\displaystyle \frac{d\tan^{-1}(x)}{dx}=\frac{1}{1+x^2}$ or by using Sympy gives, after considerable algebraic simplification $\displaystyle \frac{d\chi}{db}=-\frac{2}{\sqrt{d^2-b^2} }$. 
# 
# The differential cross section becomes $\displaystyle I(E_0,\chi) = \frac{b\sqrt{d^2-b^2}}{2\sin(\chi)}$. In the next step substitute for $b$ using the equation for $\chi$ above. This produces a difficult  looking equation $\displaystyle I(E_0,\chi)= \frac{d^2\sin(\chi/2)\cos(\chi/2)}{2\sin(\chi)}$. The trig terms simplify considerably (using the standard form $\sin(2x)=2\sin(x)\cos(x)$) and this produces the nice result $\displaystyle I(E_0,\chi)=\frac{d^2}{4}$.
...
#  
# **Q 11 answer**  Use the Verlet algorithm with values as in equation 30. The initial velocity is +10 and is positive so that the ball is thrown upwards.
...
# <img src='num-methods-fig32.png' alt='Drawing' alt='Drawing' style='width:350px;'/>
# 
