...
# **Q 22 answer** (a) Considering equations 49, if the predators $X$ do not die naturally then they will clearly consume all the prey $Y$ and the predator population will rise smoothly to a constant value and the prey uniformly fall to zero. The condition $k_3 \ll k_2Y$ ensures that the loss of the predator population $X$ is negligible.
# 
...
# 
# <img src='num-methods-fig39.png' alt='Drawing' style='width:650px'/>
# 
...
# _____
# 
# **Q 23 answer** $\displaystyle Y_e= \frac{k_3}{k_2}, \; X_e=\frac{k_1}{k_2},\; \frac{dy}{dt}=k_1y(1-x),\; \frac{dx}{dt}=k_3x(y-1)$. Changing to reduced time produces
# 
# $$\displaystyle\frac{dy}{d\tau}=y(1-x), \qquad \frac{dx}{d\tau}=\frac{k_3}{k_1}x(y-1)$$
# 
# which can be solved as outlined in section 8.1.
# 
# **Q 24 answer** The equilibrium points are the solutions to $(k_1-k_2X-k_dY)Y=0, \; (k_2Y-k_3)X=0$. Three sets of points are,
# 
...
# 
# <img src='num-methods-fig40.png' alt='Drawing' style='width:650px'/>
# 
...
# **Q 26 answer** (a) The rate equations are 
# 
...
# 
# <img src='num-methods-fig41.png' alt='Drawing' style='width:700px'/>
# 
...
# 
# <img src='num-methods-fig42.png' alt='Drawing' style='width:400px'/>
# 
...
# ______
# 
# **Q 28 answer**  Reducing the equation of motion to two equations gives $\displaystyle d\varphi/dt = v,\; dv/dt = fv+\omega^2\sin(\varphi)=0$, which using algorithm 15 produces the plot shown in figure 43.
# 
...
# 
# <img src='num-methods-fig43.png' alt='Drawing' style='width:700px'/>
# 
...
# 
# ______
# **Q 29 answer** (a) The equation is split into two parts, one describing the (angular) velocity $v$, the other the rate of change of velocity or acceleration, and which are 
# 
# $$\frac{d\varphi}{dt}=v, \qquad \frac{dv}{dt}+\left[\frac{a}{L}-\omega^2\cos(\varphi)\right]\sin(\varphi)=0$$
# 
# One pair of equilibrium points when $d\varphi/dt = dv/dt = 0$ are $v = 0$ and $\varphi = 0, \pm \pi$ which is the stationary position; $\varphi$ could be $\pi$ radians by supposing that the bar could be moved up against the shaft, which would then have to be infinitesimally thin.The other equilibrium point is $v = 0$ and $\displaystyle \varphi = \cos^{-1}\left(\frac{a}{L\omega^2}\right)$
# 
...
# 
# The nullclines are $a/L\omega^2 \to 0$ and $\varphi \to \pi/2$ or zero. The phase plane is calculated from $(\varphi,d\varphi/dt)$ which is $(\varphi, v)$.  Integrating  $dv/d\varphi = −[a/L − \omega^2 \cos(\varphi)]\sin(\varphi)/v$ gives $v$ as $\displaystyle v=\sqrt{\frac{2a}{L}\cos(\varphi)-\omega^2\cos^2(\varphi) +2c    }$
# 
...
# 
# <img src='num-methods-fig44.png' alt='Drawing' style='width:750px'/>
# 
...
# **Q 30 answer**  (a) When the angle of the rod from the vertical is small, then $\sin(\varphi) \to \varphi - \varphi^3/6$ and $\cos(\varphi) \to 1 -\varphi^2/2$. Because the angle is almost zero, the second terms are so small that they can be ignored making the equation of motion $\displaystyle \frac{d^2\varphi}{dt^2}+\left(\frac{a}{L} -\omega^2 \right)\varphi = 0$, which has the form of a simple harmonic oscillator with a frequency $\omega' = \sqrt{a/L -\omega^2}$ . This produces the phase plane on the right of Figure 44.
# 
...
# 
# **Q31 answer** Splitting the equation into two produces angular velocity and acceleration as $\displaystyle \frac{d\varphi}{dt}=v, \; \frac{dv}{dt}+\left[ \omega_0^2+h\omega^2\cos(\omega t) \right]\sin(\varphi)=0$. With the parameters given, the ratio $h\omega^2/\omega_0 = 30$, which means that the driving term, $h\omega^2\cos(\omega t)$ is going to dominate the motion. As might be expected if $h \to 0$ or $\omega \to 0$ a normal pendulum's motion is observed.
# 
# The time profile and phase plane produced are shown in Fig 45; 10<sup>3</sup> data points were used with the python  built in integrator 'odeint' but the modified Euler or Runge-Kutta method, could also be used as in Algorithm 16.
# 
# The initial angle means that the pendulum is inverted starting at $3\pi/4 \equiv 135\^\mathrm{\circ}$, and when forced by the action of the piston this angle increases with time; the pendulum crosses the vertical at a time $\approx$ 1.25 and only partly descends to the other side before re-crossing the vertical again; this motion repeats ad infinitum. The phase plane shows this oscillation about centred about $\pi$ radians. 
# 
...
# 
# <img src='num-methods-fig45.png' alt='Drawing' style='width:650px'/>
# 
...
# 
# <img src='num-methods-fig46a.png' alt='Drawing' style='width:650px'/>
# <img src='num-methods-fig46b.png' alt='Drawing' style='width:650px'/>
# 
...
# _____
# 
# **Q 33 answer** The stable or steady-state points are calculated when the derivatives of $x$ and $y$ are zero; $(a − x)(1 + x^2) - 4xy = 0$ and $\displaystyle bx\left(1-\frac{y}{1+x^2}  \right)==0$ which produces $x=a/5,\; y= 1+(a/5)^2$. The $dx/dt=0$ nullcline is $y=(a-x)(1+x^2)/4x$ and when $dy/dt = 0$ the other is $y = 1 + x^2$. These are plotted in Figure 47 together with the trajectory starting at $x = 3,\; y = 2$ with $a = 8,\; b = 2$ and in the right hand figure with $a = 10,\; b = 2$.The time profiles are also shown underneath their respective phase planes.
# 
...
# 
# <img src='num-methods-fig47a.png' alt='Drawing' style='width:700px'/>
# <img src='num-methods-fig47b.png' alt='Drawing' style='width:700px'/>
# 
