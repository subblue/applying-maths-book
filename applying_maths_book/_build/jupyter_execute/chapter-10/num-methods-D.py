...
# 
# The *stable*, *steady-state*, or *equilibrium* points are found when the rate of change is zero, and are therefore found at $dy/dt=x - x^3 =0$ and $dx/dt=y=0$ making fixed points at $x=0,\;y=0$ and $x = \pm 1,\; y = 0$. A more detailed analysis of fixed points in general shows that they may be stable, unstable or saddle points, see Jeffrey (1990) and/or Strogatz (1994). 
# 
...
# 
# 
# <img src='num-methods-fig13.png' alt='Drawing' style='width:550px;'/>
# 
...
# <img src='num-methods-fig14.png' alt='Drawing' style='width:400px;'/>
# 
...
# and changing the steps in the loop of the Euler algorithm to
...
# 
# <img src='num-methods-fig15.png' alt='Drawing' style='width:400px;'/>
# 
...
# 
# Although the Euler method will work well in all our examples, it is not necessarily the best method to use for oscillating systems, such as the pendulum, because energy is not conserved that well. By changing the algorithm 14 to

# In[ ]:


dphidt= v -> v
dvdt= phi -> -omega0^2*sin(phi)
...# define initial values and arrays etc ...
Eulerv[0]= v0
Eulerphi[0]= phi0 
for i in range(1,n):
# equations 11.39
    v  = v   + h*dvdt(phi)
    phi= phi + h*v         # Cromer changes here 
    t  = t + h
    Eulerv[i]= v
    Eulerphi[i]= phi
    dtime[i]= t
...
# then the error in the energy becomes proportional to $\Delta t^3$ which is a significant improvement over $\Delta t$ when the step size is small. The angle is calculated as phi= phi + h*v instead of phi= phi + h*dphidt as in the Euler method. See Gould et al. (2007) for more details of this and related methods.
# 
# 
# # 7 SIR equations and the spread of diseases
# 
# A very interesting, and relatively straightforward example of coupled equations is the spread of an infectious disease, because, besides being intrinsically interesting, it allows a clear illustration of a number of features such as the phase plane and nullclines. 
# 
...
# 
# Intuitively, the scheme 40 suggests that the number infected I, which is initially small (for instance, one person), increases rapidly, passes through a maximum, then slowly decays away. However, this will occur only if $k_2S_0 \gt k_1$ because when S is large, I is initially formed more rapidly than it is consumed. If the opposite is true, then I is consumed more rapidly and its population cannot become large and no epidemic occurs. To be quantitative, let $R_R = k_2S_0/k_1$ be defined as the reproductive ratio,<sup>3</sup> which is the number of secondary infections caused by one infected person if all the population is equally susceptible. An epidemic must ensue if the reproductive ratio is greater than one, because more individuals will become infected with time. This can also be appreciated by examining the rate of change of I in the second of equations 41 at $t = 0;\; dI/dt\big|_{t = 0} = I_0(k_2S_0 − k_1)$. If $k_2S_0 \gt k_1$ then $dS/dt \gt 0$ and an epidemic will occur. Typical values for the reproductive ratio are smallpox = 4; mumps = 5; German measles (rubella) = 6; measles = 12; malaria $\approx$100, (see Britton 2003).
# 
# <sup>3</sup> Many texts call the reproductive ratio $R_0$, which is unfortunately confusing with $R_0$, the initial number in the removed class.
...
# 
# Integrating with limits $I_0,\;S_0$ gives $\displaystyle \int_{I_0}^IdI=\int_{S_0}^S \left(\frac{k_1}{k_2S}-1\right)dS$ which produces
# 
...
# 
# <img src='num-methods-fig16.png' alt='Drawing' style='width:400px;'/>
# 
...
# 
# <img src='num-methods-fig17.png' alt='Drawing' style='width:400px;'/>
# 
# Figure 17. SIR data calculated for boys infected with influenza with $k_2 = 0.00218$, day<sup>-1</sup> and $k_1 = 0.452$ day<sup>-1</sup> with 763 boys one of whom was initially infected. The scheme is $\displaystyle  S+I \overset{k_2}\longrightarrow 2I ; \;   I \overset{k_1} \longrightarrow  R  $.
